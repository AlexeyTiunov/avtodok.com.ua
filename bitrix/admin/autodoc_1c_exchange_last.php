	     ...
	     elseif( $_GET["filename"] == "import.xml" )
	       {
			...
			$domxml->load($_SERVER["DOCUMENT_ROOT"]."/upload/1c_exchange/import.xml");

			$xpath = new DOMXPath($domxml);
			$nodeIsChanged = $xpath->query("//СодержитТолькоИзменения")->item(0)->nodeValue;
			...
			//echo "array of brand \n".print_r($arBrands)."\n";
			switch ($nodeIsChanged)
			{
				case "true"://проводим действия только с документом который содержит изменения
					$nodesChecked = $xpath->query("//Товар[ЗначенияСвойств/ЗначенияСвойства/Значение='Масло']");//выбираем только масла
					$queryToControllProcess= "INSERT INTO session_control_table VALUES (
																				NULL, 
																				NOW(),
																				'".serialize($nodesChecked)."')";
					$DB->Query($queryToControllProcess);
					foreach ($nodesChecked as $checked)//проходим по выбранным товарам
					{
						...
						$queryToControllProcess= "INSERT INTO session_control_table VALUES (
																					...
																					'".serialize($nodes)."')";
						$DB->Query($queryToControllProcess);
						foreach ($nodes as $node)//цикл по дочерним узлам выбранного товара
						{
		    				...
		    				{
		    					case "Бренд":
		    						$item[$nodeName] = ($arBrands[$nodeValue] != '')?$arBrands[$nodeValue]:$arBrands2[$nodeValue]; 
		    						break;
		    					case "Картинка":
		    						$pathImage = $_SERVER["DOCUMENT_ROOT"]."/upload/1c_exchange/".$nodeValue;
		    							break;
	    						default:
	    							$item[$nodeName] = $nodeValue;
	    					}
						}
						$sqlUpdate1c = "UPDATE `b_autodoc_items_1c_m` 
	    								SET 
	    									`Caption`='{$item['Наименование']}',
	    									`DATE_UPDATED` = Now()
	    							WHERE ItemCode = '{$item['Артикул']}' AND BrandCode = '{$item['Бренд']}'";//достаточно одного запроса без SELECT, как для другой базы, так как поле времени обновляется всегда
	    				$sqlUpdate = "UPDATE `b_autodoc_items_m` 
	    								SET 
	    									`Caption`='{$item['Наименование']}', 
	    									`1CRef` = '1',
	    									`type_item` = '1', 
	    									`definition` = '{$item['Комментарий']}'
	    							WHERE `ItemCode` = '{$item['Артикул']}' AND `BrandCode` = '{$item['Бренд']}'";
	    				$sqlSelect = "SELECT `id` FROM `b_autodoc_items_m` 
	    							WHERE `ItemCode` = '{$item['Артикул']}' AND `BrandCode` = '{$item['Бренд']}'";//дополнительный запрос, так как если строки идентичны запрос на обновление не затрагивает строки
	    				//echo "Запрос к базе 1С - ".$sqlUpdate1c."\n";
	    				//echo "Запрос к основной базе - ".$sqlUpdate."\n";
	    				//echo "Запрос к основной базе - ".$sqlSelect."\n";
	    				//echo "Результат 1С количество затронутых строк - ".intval($DB->Query($sqlUpdate1c,true)->AffectedRowsCount())."\n";
	    				//echo "Результат основной количество затронутых строк - ".intval($DB->Query($sqlUpdate,true)->AffectedRowsCount())."\n";
	    				//echo "Результат основной количество затронутых строк - ".intval($DB->Query($sqlSelect,true)->SelectedRowsCount())."\n";
	    				...
	    				$resS = $DB->Query($sqlSelect,true);
	    				$countReturnedRow['Select'] = intval($resS->SelectedRowsCount());
	    				
	    				$triggerInsert = true;
	    				
	    				if ($countReturnedRow['Update1c'] != 0)
	    				{
	    					$sqlTmp = "SELECT `id` FROM `b_autodoc_items_1c_m` 
	    									WHERE ItemCode = '{$item['Артикул']}' AND BrandCode = '{$item['Бренд']}'";
	    					$resTmp = $DB->Query($sqlTmp,true);
	    					$arTmp = $resTmp->Fetch();
	    					$namePicture = $arTmp['id'];
	    					$triggerInsert = false;
	    				}
	    				if ($countReturnedRow['Update'] != 0)
	    				{
	    					$sqlTmp = "SELECT `id` FROM `b_autodoc_items_m` 
	    									WHERE ItemCode = '{$item['Артикул']}' AND BrandCode = '{$item['Бренд']}'";
	    					$resTmp = $DB->Query($sqlTmp,true);
	    					$arTmp = $resTmp->Fetch();
	    					$namePicture = $arTmp['id'];
	    				...
	    				$from = imageCreateFromJPEG($pathImage);
	    				$info = @getimagesize($pathImage);
	    				$x = $info[0];
	    				$y = $info[1];
	    				
	    				//echo "\n параметры картинки - ".print_r($info);

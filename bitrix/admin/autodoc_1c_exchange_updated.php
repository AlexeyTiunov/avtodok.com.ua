	     ...
	     elseif( $_GET["filename"] == "import.xml" )
	       {
			...
			switch ($nodeIsChanged)
			{
				case "true"://проводим действия только с документом который содержит изменения
					$nodesOil = $xpath->query("//Товар[//Товар[ЗначенияСвойств//Значение='Масло']]");//выбираем только масла
					for ($i=0; $i < $nodesOil->length; $i++)
					{
						$nodes = $nodesOil->item($i)->childNodes;
						...
						do 
						{
	    					...
	    					{
	    						case "Бренд":
	    							$item[$nodeName] = ($nodeValue == 'ACURA') ? '0A' : $arBrands[$nodeValue]; 
	    							break;
	    						default:
	    					...
	    					$sqlUpdate = "UPDATE `b_autodoc_items_m` 
	    								...
	    								WHERE ItemCode = '{$item['Артикул']}' AND BrandCode = '{$item['Бренд']}'";
	    					if (!$DB->Query($sqlUpdate1c) && !$DB->Query($sqlUpdate))
	    					{

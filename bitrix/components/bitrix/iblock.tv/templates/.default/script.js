if(!jsPublicTVCollector)
	var jsPublicTVCollector = {
		...
		{
			if(
				('BUFFERING' == oldstate && 'PLAYING' ==  newstate) //flv
				|| ('Closed' == oldstate && 'Playing' ==  newstate) //wmv
				|| ('Opening' == oldstate && 'Playing' ==  newstate) //wmv
			)
			{
				if(player.getPlaylist)
					var cur_file = player.getPlaylist()[0].file; //flv
				else
					var cur_file = player.getConfig().file; //wmv
				...
				for(var file in this.files)
				{
					if(file == cur_file)
					{
						...
						CPHttpRequest.Send(TID, '/bitrix/components/bitrix/iblock.tv/stat.php', {id: this.files[file]});
...
function jsPublicTV()
{
	...
	this.Player = {wmv: null, flv:null, wmv_state_listener_added:false};
	this.PlayerConfig = {};
	...
	else if (params.type == 'flv')
	{
		...
		if(this.Player.flv && getFlashVersion()>=9) //flash 9+ version
		{
			...
			if(autoplay == true)
				...
				this.RunDelayFunction(function(){_player.sendEvent("LOAD", flvparams); _this.RunDelayFunction(function(){_player.sendEvent("STOP");}, 50, 0, 50);}, 50, 0);
		}
	}
}
...
jsPublicTV.prototype.StateListener = function(oldstate, newstate)
{
	...
	if(this.GatherStatistics && this.Player.wmv)

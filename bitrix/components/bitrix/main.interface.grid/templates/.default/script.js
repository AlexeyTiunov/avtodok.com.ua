function BxInterfaceGrid(table_id)
{
	...
	this.OnRowContext = function(e)
	{
		...
		var menu = _this.menu;
		menu.OnClose = _this.SaveColumns;
		menu.PopupHide();
		...
		if(this.IsActionEnabled('edit'))
		{
			...
			for(var i=0; i<ids.length; i++)
			{
				...
				if(el.checked)
				{
					...
					{
						if(this.oColsMeta[col_id].editable == true)
						{
	...
	this.Sort = function(url, sort_state, args)
	{
		if(sort_state == '')
		{
		...
		if(this.vars.ajax.AJAX_ID != '')
			jsAjaxUtil.InsertDataToNode(url+'&bxajaxid='+this.vars.ajax.AJAX_ID, 'comp_'+this.vars.ajax.AJAX_ID, this.vars.ajax.AJAX_OPTION_SHADOW);
		else
			window.location = url;
	}
	...
	this.SwitchFilterRow = function(row_id, menuItem)
	{
		...
		if(a.className.indexOf('bx-filter-max') != -1)
			this.SwitchFilter(a);

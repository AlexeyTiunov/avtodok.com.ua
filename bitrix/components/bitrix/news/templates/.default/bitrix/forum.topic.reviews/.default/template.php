...
if ($arResult["NAV_RESULT"]):
	if ($arParams["PAGE_NAVIGATION_TEMPLATE"] == "forum" && $arResult["NAV_RESULT"]->NavPageCount > 0):
?>
...
<?
	elseif ($arParams["PAGE_NAVIGATION_TEMPLATE"] != "forum"):
?>
...
?>
	<table cellspacing="0" border="0" class="forum-post-table <?=($iCount == 1 ? "forum-post-first " : "")?><?
		...
		?><?=($iCount%2 == 1 ? "forum-post-odd " : "forum-post-even ")?>" id="message<?=$res["ID"]?>">
		<thead><tr><td>
		<div class="forum-post-reply-buttons">
<?
	if ($arResult["FORUM"]["ALLOW_QUOTE"] == "Y"):
?>
	<a href="#review_anchor" title="<?=GetMessage("F_QUOTE_HINT")?>" class="button-small" <?
			?>onMouseDown="quoteMessageEx('<?=$res["FOR_JS"]["AUTHOR_NAME"]?>', 'message_text_<?=$res["ID"]?>')"><?=GetMessage("F_QUOTE_FULL")?></a>
<?
	endif;
?>
		<a href="#review_anchor"  title="<?=GetMessage("F_NAME")?>"  class="button-small" <?
...
?>
	<div class="forum-reply-fields">
		<div class="forum-reply-field-user">
			<div class="forum-reply-field forum-reply-field-author"><label for="REVIEW_AUTHOR<?=$arParams["form_index"]?>"><?=GetMessage("OPINIONS_NAME")?><?
...
?>
			...
			<div class="forum-reply-field forum-reply-field-email"><label for="REVIEW_EMAIL<?=$arParams["form_index"]?>"><?=GetMessage("OPINIONS_EMAIL")?><span class="forum-required-field">*</span></label>
...
<?
	endforeach;
?>
				<div class="forum-smiles-item" style="height:<?=$iMaxH?>px;padding-top:<?=$iPaddingTop?>px;">
...
<!--			<textarea name="REVIEW_TEXT" class="REVIEW_TEXT" cols="55" rows="14" tabindex="<?=$tabIndex++;?>"><?=$arResult["MESSAGE"]["REVIEW_TEXT"];?></textarea>-->
		</div>

            ...
            if(strlen($sResult)>0)
            {
                ...
                foreach($arResult[GetMessage("CC_BSC1_COM_INFO")]["#"][GetMessage("CC_BSC1_DOCUMENT")] as $value)
                {
                    if($value["#"][GetMessage("CC_BSC1_OPERATION")][0]["#"] == GetMessage("CC_BSC1_ORDER"))
                    {
                        ...
                        $arOrder[$orderId]["AGREEMENT"] =  $value["#"][GetMessage("CC_BSC1_AGREEMENT")][0]["#"];//добавляем договор
                        $arOrder[$orderId]["LOGIN"] =  $value["#"][GetMessage("CC_BSC1_LOGIN")][0]["#"];//добавляем 
                        $arOrder[$orderId]["REGION"] =  $value["#"][GetMessage("CC_BSC1_REGION")][0]["#"];//добавляем 
                        ...
                        if(is_array($value["#"][GetMessage("CC_BSC1_ITEMS")][0]["#"][GetMessage("CC_BSC1_ITEM")]))
                        {
                            foreach($value["#"][GetMessage("CC_BSC1_ITEMS")][0]["#"][GetMessage("CC_BSC1_ITEM")] as $val)
                            {
                                ...
                                $arItem[$productID] = Array(
                                        ...
                                        "ITEMSTATUSQUANTITY" => $val[GetMessage("CC_BSC1_ITEMSTATUSQUANTITY")][0]["#"],
                                        "ITEMSTATUS2" => $val[GetMessage("CC_BSC1_ITEMSTATUS2")][0]["#"],
                                        "ITEMSTATUSQUANTITY2" => $val[GetMessage("CC_BSC1_ITEMSTATUSQUANTITY2")][0]["#"] ,
                                        "ARTICLEWAS" => $val[GetMessage("CC_BSC1_ARTICLECHANGE")][0]["#"],
                                        "WAREHOUSEDATE" => $val[GetMessage("CC_BSC1_WAREHOUSEDATE")][0]["#"]
                                    );
                ...
                else
                {
                   ...
                   require_once "{$_SERVER['DOCUMENT_ROOT']}/bitrix/components/bitrix/sale.export.1c/ReceiveOrderForString.php";
                   require_once "{$_SERVER['DOCUMENT_ROOT']}/bitrix/components/bitrix/sale.export.1c/SendSMS.php";
                    ...
                    {                
                        ...
                        $statusPosition2 = $updateBasket->getStatus2ChangedPosition();
                        $statusQuantityPosition = $updateBasket->getStatusQuantityChangedPosition();
                       ...
                       # $_GET["ID"]=$ID;
                     $OrderTextObject= new ReceiveOrderForString($orderNum,$updateBasket->user);
                       
 		                $arFields = array(
			                            "USER_ID" => $updateBasket->user,
			                            "ORDER_ID" => $orderNum,
			                            "TEXT" =>$OrderTextObject->RecieveOrderText(),
                        ...
                        if (getOrderSentMailStatus($orderNum) == 0)//если не произошли изменения касающиеся всего ордера и посылается сообщение в первый раз
                        {
                        	if (count($orderChanges) > 0 || UpdateBasketITG::$statusOrderName[$orderChanges["STATUS"]["NEW"]] != "Заказ закрыт")
                        	{
                        		...
                        		{//изменился статус заказа
	                        		$event->SendImmediate( "NEW_ORDER_RECIEVED_HTML", "s1", $arFields);
                        ...
                        {//если статус не изменен и не закрыт //если статус изменен и не закрыт
                        		...
                        		|| (count($orderChanges) > 0 && UpdateBasketITG::$statusOrderName[$orderChanges["STATUS"]["NEW"]] != "Заказ закрыт"))
                        	{
                                ...
                                {
                                    $sms =new CheckMailAndSendSMS($orderNum,$userEmail,$statusPosition,$statusPosition2); 

                                      #$arFields["CHANGES"]=print_r($orderNum);  
                                    $arFields["CHANGES"] = $changesHeader.ChangedItemsToStringBuy($statusPosition,$updateBasket->currency);
                                   # $arFields["TEXT"]=GetMessage('CC_BSC1_ITEMSTATUSQUANTITY_TEXT').' ЗАКАЗАНО '.$orderPosition['QUANTITY'].'ВЫКУПЛЕНО '.$orderPosition['ITEMSTATUSQUANTITY'];  
                                    
                                    $event->SendImmediate("ORDER_STATUS_CHANGED", "s1", $arFields);
                                } 
                        		elseif (count($statusPosition2) > 0)//изменились статусы строк
                        		{
                                    
                                     $sms =new CheckMailAndSendSMS($orderNum,$userEmail,$statusPosition,$statusPosition2);
                                      #$arFields["CHANGES"]=print_r($orderNum);  
                        			$arFields["CHANGES"] = $changesHeader.ChangedItemsToStringBuy($statusPosition2,$updateBasket->currency);
                                   # $arFields["TEXT"]=GetMessage('CC_BSC1_ITEMSTATUSQUANTITY_TEXT').' ЗАКАЗАНО '.$orderPosition['QUANTITY'].'ВЫКУПЛЕНО '.$orderPosition['ITEMSTATUSQUANTITY'];  
                                    
                                    $event->SendImmediate("ORDER_STATUS_CHANGED", "s1", $arFields);
                        		} 
                                
                                 elseif (count($statusQuantityPosition) > 0)//изменилось количество статуса строки
                                {
                                    

                                      #$arFields["CHANGES"]=print_r($orderNum);  
                                    $arFields["CHANGES"] = $changesHeader.ChangedItemsToStringBuy($statusQuantityPosition,$updateBasket->currency);
                                   # $arFields["TEXT"]=GetMessage('CC_BSC1_ITEMSTATUSQUANTITY_TEXT').' ЗАКАЗАНО '.$orderPosition['QUANTITY'].'ВЫКУПЛЕНО '.$orderPosition['ITEMSTATUSQUANTITY'];  
                        		...
                        		{
			                        #$arFields["CHANGES"] = $changesHeader.replacementToStringNewP($deletePosition,$insertPosition,$updateBasket->currency);
                        			#$arFields["TEXT"]="ВНИМАНИЕ!!!  В ЗАКАЗЕ ПРОИЗВЕДЕНА ЗАМЕНА." ;
                                    $event->SendImmediate("ORDER_STATUS_CHANGED", "s1", $arFields);
                        		}
                        	}

...
else
{
	...
	#$arResult["USER_NAME"] = htmlspecialcharsEx($USER->GetFullName());
     $arResult["USER_NAME"] = htmlspecialcharsEx($USER->GetFirstName());
	$arResult["USER_LOGIN"] = htmlspecialcharsEx($USER->GetLogin());
    $ResultUserInfo= $USER->GetByLogin($arResult["USER_LOGIN"]);
    $arResult["PERSONAL_INFO"]=$ResultUserInfo->Fetch();
    $sql="SELECT ID AS MANAGER_ID, FULL_NAME AS FULL_NAME FROM `b_autodoc_managers` 
         WHERE ID=(SELECT PERSONAL_MANAGER_ID FROM `b_user` WHERE ID={$arResult["PERSONAL_INFO"]["ID"]})";
    $result=$DB->Query($sql); 
    $InfoResultArray=$result->Fetch();
    if (isset($InfoResultArray['MANAGER_ID']) && $InfoResultArray['MANAGER_ID'] !="" && $InfoResultArray['MANAGER_ID']!=null)
    {
        $arResult["MANAGER_INFO"]=$InfoResultArray;
        
        
    } else
    {
        $sql="SELECT ID AS MANAGER_ID, FULL_NAME AS FULL_NAME FROM  b_autodoc_managers WHERE DEFAULT_V=1";
        $result=$DB->Query($sql);
        $arResult["MANAGER_INFO"]=$result->Fetch();
        #var_dump($result->Fetch());
        
    }     
    
    $sql="SELECT * FROM b_user_extrainfo WHERE ID={$arResult["PERSONAL_INFO"]["ID"]}";
    $result=$DB->Query($sql);
    $InfoResultArray=$result->Fetch();
    if ($InfoResultArray['MONTH_TURNOVER']!="onebuyR")
    {
      $arResult["FILENAME"]="Prohorov";  
    }  
    else
    {
       $arResult["FILENAME"]="Nikitin"; 
    }
    /*if ($arResult["PERSONAL_INFO"]["PERSONAL_STATE"]!="")
     {
        $sql="SELECT* FROM  b_user_regions WHERE ID={$arResult["PERSONAL_INFO"]["PERSONAL_STATE"]}" ;
        $result=$DB->Query($sql);
        $RegionResultArray=$result->Fetch();
        if ($RegionResultArray["EMAIL"]!="")
        {
            $arResult["FILENAME"]="Nikitin";
        } else
        {

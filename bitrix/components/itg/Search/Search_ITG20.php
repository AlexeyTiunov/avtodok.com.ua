...
class Search_ITG
{
	...
	private function getArray()
	{
		...
		while ($arItem = $this->tmpRes->fetch_assoc())
		{
			...
			if ($arItem['RegionShortName'] == 'СКЛАД') $this->params['warehouse'] = true;
			$this->items[] = $arItem;
	...
	private function prepareSql()
	{
		...
		$this->params['fieldsUnion'] = " 
									...
									`RegionCode`, 
									`PriceCode`, 
									`Price`*(SELECT IF(`RegionCode`=1,1,(SELECT ITG_region_koef(`RegionCode`,'{$this->params['user']}')))) AS Price,
									(`Price`*
										(SELECT IF(`RegionCode`=1,1,(SELECT ITG_region_koef(`RegionCode`,'{$this->params['user']}')))))*
		...
		$this->sql['exact'] = "SELECT ".$this->params['fieldsUnion']." FROM `ITG_price_union` AS ITG
										WHERE ".$this->params['bcode']." 
											(`RegionCode`!=1 OR (`RegionCode`=1 AND `PriceCode`=(SELECT ITG_price_koef('{$this->params['user']}'))))
											AND
											`ItemCode`='{$this->params['icode']}'

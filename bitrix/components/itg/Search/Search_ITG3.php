...
class Search_ITG
{
    ...
    private function prepareSql()
    {
        ...
        $this->sql['exact'] = "SELECT ".$this->params['fieldsUnion']." FROM `ITG_price_union` AS ITG
                                        WHERE ".$this->params['bcode']." 
                                            (`RegionCode`!=1 OR (`RegionCode`=1 AND `PriceCode`=(SELECT ITG_price_koef('{$this->params['user']}'))) OR (`RegionCode`=1000 AND `PriceCode`=(SELECT ITG_price_koef('{$this->params['user']}'))))
                                            AND
                                            `ItemCode`='{$this->params['icode']}'

    ...
    function __construct($params)
    {
       ...
       # $selectPointOfReference['Begin']=20100101;
        ...
        if ($this->params['dateBegin'])
        {
            ...
            $expencesBegin = $resExpencesBegin->Fetch();         //Расходы
        }
        $sqlSelectExpencesEnd = "    SELECT SUM(`".self::SUMM_SHIPMENT."`)  
                                        FROM `".self::NAME_TABLE_SHIPMENT."` 
                                                WHERE  
                                                        `".self::AGREEMENT_CODE_SHIPMENT."`='".$this->params['agreement']."' 
                                                    AND `".self::CLIENT_CODE_SHIPMENT."`='".$this->params['user']."' 
        ...
        if ($this->params['dateBegin'])
        {
            $sqlSelectParishesBegin = "SELECT IF('{$selectPointOfReference['Begin']}' < '{$this->params['dateBegin']}',
                                            ...
                                            FROM `".self::NAME_TABLE_PAY."` 
                                                    WHERE  
                                                        ...
                                                        AND CAST(CAST(`".self::DATE_DOCUMENT_PAY."` as DATE) as SIGNED)<'{$this->params['dateBegin']}'),0.00 )";
            ...
            $parishesBegin = $resParishesBegin->Fetch();         //Оплаты
        }
        $sqlSelectParishesEnd = "   SELECT '{$selectPointOfReference['BalanceOnDate']}'+SUM(`".self::SUMM_PAY."`) 
                                        FROM `".self::NAME_TABLE_PAY."` 
                                                WHERE  
                                                        `".self::AGREEMENT_CODE_PAY."`='".$this->params['agreement']."' 
                                                    AND `".self::CLIENT_CODE_PAY."`='".$this->params['user']."' 
       ...
       /* //echo "ptp:{$selectPointOfReference['BalanceOnDate']}<br/>";
        ...
        else 
        {
         ...
         $this->balance['begin'] =$selectPointOfReference['BalanceOnDate']-$expBegin+$parBegin ; 
         $this->balance['end'] =1;
          $this->balance['Debt'] =$expEnd-$parEnd;  
    }

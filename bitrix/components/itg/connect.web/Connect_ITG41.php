           ...
           # $this->prepareResult();                       
            ...
            else           
            {
             ...
             try 
                {  
                 $this->params['client'] = $name;
                 $this->client = @new SoapClient(self::$log[$this->params['client']]['url'], array('encoding'=>self::$log[$this->params['client']]['encoding'],'connection_timeout'=>5));                                                                                           
                }catch (SoapFault $ex)
                {
		...
		foreach ($this->result as $key=>$product)
		{
		   ...
		   //if (isset($product['Quantity']) && intval($product['Quantity']) == 0) continue; 
            ...
            //if ((isset($product['Quantity']) && intval($product['Quantity']) == 0)||(isset($product["SupplierCode"]) && iconv  ('utf8','cp1251',$product["SupplierCode"])!="KLOD" && iconv('utf8','cp1251',$product["SupplierCode"])!="USAF") )
                 //continue;
if ((isset($product['Quantity']) && intval($product['Quantity']) == 1000)||(isset($product["SupplierCode"])  && $product["SupplierCode"]!="USAF" && $product["SupplierCode"]!="USFF" && $product["SupplierCode"]!="USAM" ) ) continue;     //TechnoMir
		...
		#echo "</pre>";
            ...
            $product['REGION'] = self::$log[$this->params['client']]['region'];     
                ...
                case 'tehnomir':
                    ...
                    $product['REGIONR']=($product['SupplierCode']=='USFF' or $product['SupplierCode']=='USAF' or $product['SupplierCode']=='USAM' ) ? 'USA'  :'УКРАИНА' ;
            ...
            $product['PRICEREGIONP'] = ($product['SupplierCode']=='USFF' or $product['SupplierCode']=='USAF' or $product['SupplierCode']=='USAM') ?$product['Price']-($product['Price']*0.03): /*$sqlRes['koefPrice']**/$product;

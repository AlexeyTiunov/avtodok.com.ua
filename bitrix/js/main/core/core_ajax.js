...
var
	...
	defaultConfig = {
		...
		timeout: 60, // request timeout. 0 for browser-default
...
BX.ajax.submit = function(obForm, callback)
{
	if (!obForm.target) 
	{
		if (null == BX.ajax.formTarget)
		{
			BX.ajax.formTarget = document.body.appendChild(BX.create('IFRAME', {
				props: {
					name: 'formTarget',
					id: 'formTarget',
		...
		obForm.target = BX.ajax.formTarget.name;
		
		BX.bind(BX.ajax.formTarget, 'load', function()
		{
			if (callback)
				callback(BX.ajax.formTarget.contentWindow.document.body.innerHTML);
	...
	if (!obForm.submitbtn)
	{
		obForm.submitbtn = obForm.appendChild(BX.create('INPUT', {
			'props': {
				'type': 'submit',
				'name': 'save',
				'value': 'Y'

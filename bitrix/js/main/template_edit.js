...
function preview_template(ID, sess_id)
{
	...
	try{GLOBAL_pMainObj['CONTENT'].onSubmit();}catch(e){}
	var _tp = new JCHttpRequest();
	_tp.Action = function(result)
	{
		CloseWaitWindow();
		setTimeout(function ()
			{
	...
	ShowWaitWindow();
	var _c = decodeContentFromLCA(content);
	postData = 'CONTENT='+encodeURIComponent(_c);
	postData += '&STYLES='+encodeURIComponent(document.getElementById('__STYLES').value);
	postData += '&TEMPLATE_STYLES='+encodeURIComponent(document.getElementById('__TEMPLATE_STYLES').value);
	postData += '&'+sess_id;
	
	_tp.Post('/bitrix/admin/template_create_preview.php?ID='+ID, postData);
}
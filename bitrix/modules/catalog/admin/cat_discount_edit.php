...
function CatalogGenerateCoupon()
{
	...
	if ($arEvent = $events->Fetch())
		return ExecuteModuleEvent($arEvent);
	...
	<tr>
		...
		<td valign="top" width="60%">
			<table cellpadding="0" cellspacing="0" border="0" id="products_table">
				...
				<tr>
					<td>
						<input type="button" value="<?= GetMessage("DSC_ADD_PROD") ?>" onClick="window.open('cat_product_search.php?new_value=Y&amp;form_name=fdiscount_edit&amp;func_name=InsertProduct&amp;lang=<?= htmlspecialchars($lang) ?>', '', 'scrollbars=yes,resizable=yes,width=600,height=500,top='+Math.floor((screen.height - 500)/2-14)+',left='+Math.floor((screen.width - 600)/2-5));">
					</td>
				</tr>
			</table>
			...
			function InsertProduct(id, name)
			{
				...
				oCell.innerHTML = 
					...
					'&amp;form_name=fdiscount_edit&amp;lang=<?= htmlspecialchars($lang) ?>\', ' +

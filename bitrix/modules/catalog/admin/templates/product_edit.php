...
function getElementForm()
{
	for(var i = 0; i < document.forms.length; i++)
		if(document.forms[i].name.substring(0, 12) == 'form_element')
		...
		<table border="0" cellspacing="0" cellpadding="0" width="100%" class="edit-table" id="catalog_price_table">
			<tr>
				<td width="40%">
					...
					?>
					<span class="required">*</span><?echo GetMessage("BASE_PRICE")?> (<?echo $arBaseGroup["NAME"] ?>):
				</td>
				<td width="60%">
					<script type="text/javascript">
			...
			<tr>
				<td>
					<span class="required">*</span><?echo GetMessage("BASE_CURRENCY")?>:
				</td>
				<td>
					<?
					if ($arBasePrice)
...
<div id="prices_ext" style="display: <?=$bUseExtendedPrice ? 'block' : 'none'?>;">
		...
		function CloneOtherPriceGroup(ind)
		{
			...
			{
				?>
				str += '<option value="<?= $GLOBALS["MAIN_EXTRA_LIST_CACHE"][$i]["ID"] ?>"><?= htmlspecialchars($GLOBALS["MAIN_EXTRA_LIST_CACHE"][$i]["NAME"])." (".htmlspecialchars($GLOBALS["MAIN_EXTRA_LIST_CACHE"][$i]["PERCENTAGE"])."%)" ?></option>';
				<?
			}
			?>
			str += '</select>';

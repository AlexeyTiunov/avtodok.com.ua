...
$arCatalogAvailValueFields = array(
		/*array("value"=>"CV_QUANTITY_FROM", "field"=>"QUANTITY_FROM", "important"=>"N", "name"=>"Покупаемое количество от"." (B_CATALOG_PRICE.QUANTITY_FROM)"),
		array("value"=>"CV_QUANTITY_TO", "field"=>"QUANTITY_TO", "important"=>"N", "name"=>"Покупаемое количество до"." (B_CATALOG_PRICE.QUANTITY_TO)"),*/
...
function CatalogPayOrderCallback($productID, $userID, $bPaid, $orderID)
{
	...
	if ($arIBlockElement = $dbIBlockElement->GetNext())
	{
		...
		if ($arCatalog["SUBSCRIPTION"] == "Y")
		{
			...
			// Записываем на recurring, если нужно

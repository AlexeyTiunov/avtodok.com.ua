	...
	switch ($name)
	{
		case $nameUTF["Catalog"]:
			...
			$dbIBlockList = CIBlock::GetList(
					array(),
					array("=XML_ID" => $currentCatalog["ID"], 'MIN_PERMISSION' => 'W')
				);
		...
		case $nameUTF["Category"]:
			if ($currentCatalog)
			{
				...
				$strSql =
					"INSERT INTO b_catalog_cml_section (XML_ID, CATALOG_ID, PARENT_XML_ID, NAME) ".
					"VALUES ('".$currentCategory["ID"]."', ".$currentCatalog["BID"].", '".$currentCategory["ParentCategory"]."', '".$DB->ForSql($currentCategory["Name"])."')";
		...
		case $nameUTF["Product"]:
			if ($currentCatalog)
			{
				...
				$strSql =
					"INSERT INTO b_catalog_cml_product (XML_ID, CATALOG_ID, NAME, MODIFIED_BY, PARENT_CATEGORY) ".
					"VALUES ('".$currentProduct["ID"]."', ".$currentCatalog["BID"].", '".$DB->ForSql($currentProduct["Name"])."', ".((IntVal($GLOBALS["USER"]->GetID()) > 0) ? IntVal($GLOBALS["USER"]->GetID()) : 1).", '".$currentProduct["ParentCategory"]."')";
...
function __getMemoryUsage()
{
	...
	if (substr(PHP_OS, 0, 3) == 'WIN')
	{
		...
		return trim(substr($output[$resultRow], $resultRowItemStartPosition, $resultRowItemLength));
	}
	*/
	...
	if ($iBlockIDString != "0")
	{
		...
		if ($keepExistingData == "N")
		{
			...
			while ($arRes = $dbRes->Fetch())
			{
				...
				{
					// ÅÑËÈ Ó ÑÊÈÄÊÈ ÁÎËÜØÅ ÍÅÒ ÔÈËÜÒÐÎÂ ÏÎ ÃÐÓÏÏÅ - ÄÅÀÊÒÈÂÈÐÎÂÀÒÜ!!!
					$strSql =
		...
		// Update sections
		$strSql =
			"SELECT T.ID, X.NAME, X.PARENT_XML_ID ".
			"FROM b_catalog_cml_tmp T ".
		...
		// Add new sections
		$strSql =
			"INSERT INTO b_iblock_section(IBLOCK_ID, ACTIVE, GLOBAL_ACTIVE, SORT, NAME, XML_ID, TMP_ID) ".
			"SELECT T.CATALOG_ID, 'Y', 'Y', 500, X.NAME, X.XML_ID, '".$DB->ForSql($tmpid)."' ".
			"FROM b_catalog_cml_tmp T ".
			"	INNER JOIN b_catalog_cml_section X ON (T.XML_ID = X.XML_ID AND T.CATALOG_ID = X.CATALOG_ID) ".
			"WHERE T.ID = 0 ";
		$DB->Query($strSql);
		...
		if ($keepExistingData == "N")
		{
			...
			while ($arRes = $dbRes->Fetch())
			{
				...
				if ($num > CML_GROUP_OPERATION_CNT)
				{
					// ÅÑËÈ Ó ÑÊÈÄÊÈ ÁÎËÜØÅ ÍÅÒ ÔÈËÜÒÐÎÂ ÏÎ ÒÎÂÀÐÓ - ÄÅÀÊÒÈÂÈÐÎÂÀÒÜ!!!
					$strSql =
		...
		// Update products
		$strSql =
			"SELECT T.ID, X.NAME, X.MODIFIED_BY, S.ID as PARENT_CATEGORY, MIN(S1.ID) as CNT ".
			"FROM b_catalog_cml_tmp T ".
			...
			"WHERE T.ID > 0 ".
			"GROUP BY T.ID, X.NAME, X.MODIFIED_BY, S.ID";
		$dbRes = $DB->Query($strSql);
		while ($arRes = $dbRes->Fetch())
		{
			$strSql =
		...
		// Add new products
		$strSql =
			"INSERT INTO b_iblock_element(IBLOCK_ID, ACTIVE, MODIFIED_BY, CREATED_BY, SORT, NAME, XML_ID, IBLOCK_SECTION_ID, IN_SECTIONS, TMP_ID) ".
			"SELECT T.CATALOG_ID, 'Y', X.MODIFIED_BY, X.MODIFIED_BY, 500, X.NAME, X.XML_ID, S.ID, IF(MIN(S1.ID), 'Y', 'N'), '".$DB->ForSql($tmpid)."' ".
			"FROM b_catalog_cml_tmp T ".
			"	INNER JOIN b_catalog_cml_product X ON (T.XML_ID = X.XML_ID AND T.CATALOG_ID = X.CATALOG_ID) ".
			"	LEFT JOIN b_iblock_section S ON (S.XML_ID = X.PARENT_CATEGORY AND X.CATALOG_ID = S.IBLOCK_ID) ".
			"	LEFT JOIN b_catalog_cml_product_cat XC ON (T.CATALOG_ID = XC.CATALOG_ID AND T.XML_ID = XC.PRODUCT_XML_ID) ".
			"	LEFT JOIN b_iblock_section S1 ON (S1.XML_ID = XC.CATEGORY_XML_ID AND XC.CATALOG_ID = S1.IBLOCK_ID) ".
			"WHERE T.ID = 0 ".
			"GROUP BY T.CATALOG_ID, X.MODIFIED_BY, X.NAME, X.XML_ID, S.ID";
		$DB->Query($strSql);

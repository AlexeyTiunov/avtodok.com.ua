...
$startImportExecTime = getmicrotime();
     ...
     //  Вырезанные куски
$PATH2IMAGE_FILES = "";
$outFileAction="F"; // Товары, которых не было в файле - не трогать
$inFileAction=="F"; // Неактивные товары / группы, которые есть в файле - оставить как есть
...
if (strlen($URL_DATA_FILE) > 0)
{
	...
	if (file_exists($_SERVER["DOCUMENT_ROOT"].$URL_DATA_FILE) && is_file($_SERVER["DOCUMENT_ROOT"].$URL_DATA_FILE) && $GLOBALS["APPLICATION"]->GetFileAccessPermission($URL_DATA_FILE)>="W")
	...
	if ($arRes)
	{
		...
		*/
		...
		if ($bAllLinesLoaded)
		{
			...
			*/
			...
			while($arr = $res->Fetch())
			{
				...
				{
					$el->Update($arr["ID"], Array("ACTIVE" => "N"), false, false);
					$killed_lines++;
				}
			}

			if ($outFileAction=="D" || $outFileAction=="H")
			{
				$res = CPrice::GetList(
						array(),
		...
		else
		{
			...
			*/
			$INTERNAL_VARS_LIST = "tmpid,line_num,correct_lines,error_lines,killed_lines,arIBlockProperty,bThereIsGroups,arProductGroups,arIBlockPropertyValue";

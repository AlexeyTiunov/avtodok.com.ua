...
if (isset($_GET['check_for_file']) && $_GET['check_for_file'] == 'Y' && is_file($absPath))
		...
		else
		{
			...
			else
			{
				...
				else
				{
					...
					elseif (!file_exists($documentRoot.$pathFrom))
					{
					...
					elseif(!@rename($documentRoot.$pathFrom, $documentRoot.$pathTo))
					{
...
if(!$bSearch && strlen($path) > 0 && ($logical != "Y" || rtrim($arSite["DIR"], "/") != rtrim($arParsedPath["FULL"], "/")))
{
	...
	if($logical == "Y")
		$showField = "<a href=\"javascript:".$sTableID.".GetAdminList('fileman_admin.php?".$addUrl_s."&site=".$site."&path=".urlencode($arParsedPath["PREV"])."&show_perms_for=".IntVal($show_perms_for)."', GALCallBack);\"><IMG SRC=\"/bitrix/images/fileman/types/folder_up.gif\" WIDTH=\"16\" HEIGHT=\"16\" BORDER=0 alt=\"".GetMessage("FILEMAN_UP")."\"></a>&nbsp;<a href=\"javascript:".$sTableID.".GetAdminList('fileman_admin.php?".$addUrl_s."&site=".$site."&path=".urlencode($arParsedPath["PREV"])."&show_perms_for=".IntVal($show_perms_for)."', GALCallBack);\">..</a>";
	else
		$showField = "<a href=\"javascript:".$sTableID.".GetAdminList('fileman_admin.php?".$addUrl_s."&site=".$site."&path=".urlencode($arParsedPath["PREV"])."&show_perms_for=".IntVal($show_perms_for)."', GALCallBack);\"><IMG SRC=\"/bitrix/images/fileman/types/folder_up.gif\" WIDTH=\"16\" HEIGHT=\"16\" BORDER=0 alt=\"".GetMessage("FILEMAN_UP")."\"></a>&nbsp;<a href=\"javascript:".$sTableID.".GetAdminList('fileman_admin.php?".$addUrl_s."&site=".$site."&path=".urlencode($arParsedPath["PREV"])."&show_perms_for=".IntVal($show_perms_for)."', GALCallBack);\">..</a>";
	...
	if ($logical != "Y")
	{
		...
		if ($USER->CanDoFileOperation('fm_edit_permission',$arPath))
		{
			...
			$arActions[] = array(
				...
				"ACTION" => "setCheckbox('".Cutil::JSEscape($f_NAME)."'); setAccess('".Cutil::JSEscape($site)."', '".Cutil::JSEscape($path)."');"
			);
...
if($USER->CanDoFileOperation('fm_edit_permission',$arPath))
{
	$arGrActionAr["access"] = array(
		"action" => "setAccess('".Cutil::JSEscape($site)."', '".Cutil::JSEscape($path)."')",
		"value" => "access",
...
<tr>
	...
	<td style="padding-left:3px; padding-right:3px;"><input class="form-button" type="button" value="OK" title="<?= GetMessage("FILEMAN_FAST_PATH_BUTTON")?>" OnClick="<?= $sTableID ?>.GetAdminList('fileman_admin.php?<?=$addUrl?>&site=<?= urlencode($site) ?>&path='+BX('quick_path').value+'&show_perms_for=<?= IntVal($show_perms_for) ?>&check_for_file=Y', GALCallBack)"></td>
</tr>
</table>

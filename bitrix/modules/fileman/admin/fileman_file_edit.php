...
<form method="POST" action="<?echo $APPLICATION->GetCurPage()?>?" name="ffilemanedit">
...
$tabControl->BeginNextTab();?>
	...
	<?if(!$bEdit):?>
		...
		<tr>
			...
			<td width="70%">
				<select name="template" onchange="window.location='/bitrix/admin/fileman_file_edit.php?lang=<?echo LANG?>&site=<?=Urlencode($site)?>&path=<?= UrlEncode($path)?><? echo ($full_src=="Y" ? "&full_src=Y" : "")?>&new=y&template='+escape(this[this.selectedIndex].value)">
					<?for($i=0; $i<count($arTemplates); $i++):?>
	...
	<?if(!$bFullPHP):?>
		<tr>
			<td width="30%"><?echo GetMessage("FILEMAN_FILEEDIT_TITLE")?></td>
			<td width="70%">
				<input type="text" name="title" size="50" maxlength="255" value="<?echo htmlspecialchars($title)?>">
			</td>
		</tr>
		<?if($bEditProps):?>
	...
	<?endif?>
		<tr><td colspan="2">
			<textarea name="filesrc" rows="37" style="width:100%; overflow:auto;" wrap="OFF"><?echo htmlspecialchars($filesrc)?></textarea></td></tr>

...
if((strlen($new) <= 0 || strtolower($new) != 'y' || strlen($filename) <= 0) && !file_exists($abs_path))
{
	$p = strrpos($path, "/");
	if($p!==false)
	{
...
{
	if(strlen($new) >0 && strtolower($new) == 'y' && strlen($filename) > 0 && file_exists($abs_path))// если мы хотим создать новый файл, но уже такой есть - ругаемся
	{
	...
	if($REQUEST_METHOD=="POST" && strlen($save)>0 && strlen($propeditmore)<=0)
	{
		...
		else
		{
			if($limit_php_access)
			{
				...
				$old_res = CFileman::ParseFileContent($APPLICATION->GetFileContent($abs_ofp), true);
...
?>
	...
	<?if(!$bEdit):?>
		...
		<?$arTemplates = CFileman::GetFileTemplates(LANGUAGE_ID, array($site_template));?>
		<tr><td><?= GetMessage("FILEMAN_FILEEDIT_TEMPLATE")?></td>
		<td>
		<script>
		function templateOnChange(_this)
		{
			var _name = document.getElementById('filename').value;
			if (_name)
		...
		?>
		<select name="template" onchange="templateOnChange(this);">
			<?for($i=0; $i<count($arTemplates); $i++):?>
		...
		<tr valign="top"><td><?= GetMessage("FILEMAN_FILEEDIT_NAME")?></td>
		<td>
			<?if (isset($filename2))
				$filename = $filename2;?>
			<input type="text" name="filename" id="filename" style="float: left;" size="30" maxlength="255" value="<?= htmlspecialchars($filename)?>">
			...
			<script>
			var oInput = document.getElementById('filename');
	...
	<?endif?>
	<tr>
		<td width="30%"><?= GetMessage("FILEMAN_FILEEDIT_TITLE")?></td>
	...
	<td colspan="2">
		...
		{
			var bOldPath = (document.getElementById('path').value == path);
			<?if(!$bEdit):?>
			...
			<?else:?>
				document.getElementById('path').value = path;
				document.getElementById('filename').value = filename;
				document.getElementById('new').value = "y";
			<?endif?>
			...
			if(!menu['type'])
				document.getElementById('add_to_menu').checked = false;
			else
			{
				document.getElementById('add_to_menu').checked = true;
				document.getElementById('menutype').value = menu['type'];
				__heAddToMenu();
				chtyp();
				document.getElementById('itemtype_n').disabled = false;
				document.getElementById('itemtype_e').disabled = false;
				if(menu['menu_add_new'])
				{
					document.getElementById('itemtype_n').checked = true;
					chitemtype();
					document.getElementById('newp').value = menu['menu_add_name'];
					if(!bOldPath)
					{
						while(document.getElementById('newppos').length>0)
							document.getElementById('newppos').remove(0);

						var oOption = new Option(menu['menu_add_pos'], menu['menu_add_pos'], false, false);
						document.getElementById('newppos').options.add(oOption);
					}
					else
						document.getElementById('newppos').value = ""+parseInt(menu['menu_add_pos']);
				}
				else
				{
					document.getElementById('itemtype_n').checked = false;
					document.getElementById('itemtype_e').checked = true;
					chitemtype();
					if(!bOldPath)
					{
						while(document.getElementById('menuitem').length>0)
							document.getElementById('menuitem').remove(0);
						var oOption = new Option(menu['menu_add_pos'], menu['menu_add_pos'], false, false);
						document.getElementById('menuitem').options.add(oOption);
					}
					else
						document.getElementById('menuitem').value = ''+parseInt(menu['menu_add_pos']);
				}
			}

			document.getElementById("filesrc").pMainObj.SaveContent(true);
			if(apply)
				document.getElementById("apply2").value = 'Y';
			document.getElementById("filesrc").pMainObj.isSubmited = true;
			document.getElementById("filesrc").pMainObj.pForm.submit();
		}
		</script>
		<?
			...
			<table border="0" cellspacing="1" cellpadding="3" id="proptab"  class="internal">
				...
				<tr>
					<td colspan="2">
						...
						<input type="button" name="propeditmore"  value="<?echo GetMessage("FILEMAN_EDIT_PROPSMORE")?>" onClick="_MoreRProps()">
					</td>
				</tr>
			</table>
		</td>
...
{
	var add_to_menu = document.getElementById("add_to_menu");
	if(add_to_menu.checked)
	{
		__CHRow(document.getElementById("ex"));
		__CHRow(document.getElementById("e0"));
		__CHRow(document.getElementById("e1"));
		__CHRow(document.getElementById("e2"));
		__CHRow(document.getElementById("e3"));
		chtyp();
	}
	else
	{
...
{
	var cur = document.getElementById("menutype")[document.getElementById("menutype").selectedIndex].value;
	var i;
	...
	list = document.getElementById("newppos");
	while(list.length>0)
		list.remove(0);
	for(i=0; i<itms.length; i++)
	{
...
function chitemtype()
{
	...
	var ffilemanedit = document.getElementById("ffilemanedit");
	var itms = arItems[i];
	if(itms.length<=0)
	{

...
if (isset($_GET['action']) && $_GET['action'] == 'upload' && check_bitrix_sessid())
{
	...
	die('#JS#&first_id='.$firstId.'&col_id='.$collectionId.'&ml_type='.$_GET['ml_type'].'#JS#');
}
	...
	if ($i > 0)
	{
		...
		function addCollToItem(ItemId, id, pSel)
		{
			...
			var
				...
				pDiv = jsUtils.CreateElement("DIV", {className: 'mu-check-col', title: oCol.NAME}),
				pSpan = pDiv.appendChild(jsUtils.CreateElement("SPAN")),

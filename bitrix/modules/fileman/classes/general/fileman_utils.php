	...
	function InitScript($Params)
	{
		...
		window.SearchReplaceRun = function(path, bSearch, ssess, lastValues)
		{
			...
			if (!window.oFMSearch || !window.oFMSearch.bInited)
			{
				window.oFMSearch = new BXFMSearch(
				{
					...
					viewFilePath: '<?= $Params['viewFilePath']?>',
					viewFolderPath: '<?= $Params['viewFolderPath']?>',
		...
		window.CopyMoveRun = function(arFiles, bCopy)
		{
			...
			window.oBXFMCopy.OnOpen({
				...
				ssess: "<?= CUtil::JSEscape($_GET['ssess'])?>"
			});
		};
		<?endif;?>
...
<?
	...
	function BuildDialogContent()
	{
		...
		$defMask = COption::GetOptionString($module_id, "search_mask", "*.php");
		// Search dialog
		...
		$searchTabControl = new CAdmintabControl("searchTabControl", $aTabs, false);
		...
		<table  id="bx_fms_tbl" class="bxfm-d-params bxfm-d-params-add-hide">
			...
			<tr title="<?= GetMessage("FM_UTIL_DIR_TITLE")?>">
				...
				<td class="bxfm-d-value"><input id="bx_search_dir" name="bx_search_dir" value=""  style="width: 220px;"/><input id="bx_search_fd_but" type="button" value="..." title="<?= GetMessage('FD_OPEN_DIR')?>" onclick="FMFD_SearchDirOpen();" /></td>
			</tr>
			<tr>
...
?>
		...
		<table class="bx-copy-cont-tbl bx-copy-cont-tbl-add-hide" id="bx_copy_table">
			<tr>
			...
			<tr>
				...
				<td class="bxfm-d-value"><input id="bx_copy_to" style="width: 260px;" value="/" /><input type="button" value="..." title="<?= GetMessage('FD_OPEN_DIR')?>" onclick="FMFD_CopyMoveOpen();"  /></td>
			</tr>
...
?>
		...
		<div style="margin: 10px 20px 5px 20px; width: 460px;">
...
class CFilemanSearch
{
	function Init($Params)
	{
		...
		if ($this->Params['bInResult'])
		{
			...
			{
				$path = $_SERVER["DOCUMENT_ROOT"].$searchRes[$i]['path'];
				if ($this->CheckBreak())
				{
					$bBreak = true;
					$nextPath = $path;
...
<?if ($bBreak):  // Execution breaks on timeout?>
	...
	window.fmsLastPath = '<?= CUtil::JSEscape(CFilemanUtils::TrimPath($nextPath))?>';
<? else: ?>
...
<?
	...
	function Search($file)
	{
		...
		$path = CFilemanUtils::TrimPath($file);
		...
		if ($this->Params['fileName'] != "")
		{
			...
			else // name pattern with "*"
			{
				...
				$pattern = str_replace('*', '.*', $pattern);
				if (!preg_match('/^'.$pattern.'$/i', basename($file)))
					return;
			}
		}
		...
		if ($this->Params['phrase'] != "")
		{
			...
			$isPHP = CFileman::IsPHP($fileContent) || in_array(CFileman::GetFileExtension($path), CFileMan::GetScriptFileExt()) || substr($name, 0, 1) == ".";
...
class CFilemanUtilDir
{
	...
	function Recursion($dir)
	{
		...
		{
			$this->err[] = 'Error on open folder: '.$dir;
			return false;
		}
		...
		closedir($handle);
		sort($arItems);
		$s_dir = CFilemanUtils::TrimPath($dir);
...
class CFilemanChmod
{
	...
	function Chmod($file)
	{
		...
		//echo "chmod:".$file." >> ".$this->value."\n\n";
		//$this->Result[] = array(CFilemanUtils::TrimPath($file), chmod($file, $this->value));
...
class CFilemanCopy
{
	function Init($Params)
	{
		...
		if (count($arWarnings) == 0)
		{
			...
			foreach ($Params['arFiles'] as $filePath)
			{
				...
				$bDir_i = is_dir($absPath_i);
				$name_i = CFileman::GetFileName($filePath);
				$strWarn = "";

				// Check if file already exists in destination folder
				if (file_exists($absPathTo.$name_i) && $bDir_i == is_dir($absPathTo.$name_i))
				{
					...
					if ($caseOption == 'ask')
					{
						...
						window.BXFM_fileExist = {
							fileOld: {
								...
								size: "<?= $bDir_i ? '-' : CFileMan::GetStrFileSize(filesize($absPathTo.$name_i))?>",
								date: "<?= date(CDatabase::DateFormatToPHP(CLang::GetDateFormat("FULL")), filemtime($absPathTo.$name_i))?>"
							},
							fileNew: {
								alt_name: "<?= $altName?>",
								name: "<?= $name_i?>",
								path: "<?= $filePath?>",
								bDir: <?= $bDir_i ? "true" : "false"?>,
								size: "<?= $bDir_i ? '-' : CFileMan::GetStrFileSize(filesize($absPath_i))?>",
								date: "<?= date(CDatabase::DateFormatToPHP(CLang::GetDateFormat("FULL")), filemtime($absPath_i))?>"
							}
						};
						</script><?
						return;
	...
	function GetAltFileName($absPath, $name, $bDir = false)
	{
		for ($i=1; $i <= 9999; $i++)
		{
			...
			if (!file_exists($absPath.$new_name))
				break;
		}
		return $new_name;
	}

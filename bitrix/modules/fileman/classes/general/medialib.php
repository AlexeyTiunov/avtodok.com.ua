...
div.bx-ml-pnbutton div.bx-pn2{background: url(/bitrix/images/fileman/medialib/browse.gif) -14px 0; width: 10px; height: 21px; float: left;}
 div.bx-ml-pnbutton div.pressed{background: url(/bitrix/images/fileman/medialib/browse.gif) -39px 0;}
...
if (!window.<?= $cid?>_menu)
{
	...
	function <?= $cid?>_onclick(pEl){window.<?= $cid?>_menu.ShowMenu(pEl, null, false, {top: 0,bottom: 0,left: -12, right:0});}
}
</script>
...
<?
	...
	/*
		...
		$strImageID // идентификатор файла
		$showInfo => array(
			"IMAGE" => "Y",
	...
	*/
	...
	)
	{
		...
		if($arFile)
		{
			...
			$sImagePath = "/".$strImageStorePath."/".$arFile["SUBDIR"]."/".$arFile["FILE_NAME"];
			$sImagePath = str_replace("//","/",$sImagePath);
			if(file_exists($_SERVER["DOCUMENT_ROOT"].$sImagePath))
			{
				$bFileExists = true;
			...
			if(is_array($showInfo))
			{
				...
				if($bFileExists)
				{
						...
						if($iMaxW > 0 && $iMaxH > 0) //need to check scale, maybe show actual size in the popup window
						{
							...
							if($intWidth > $iMaxW || $intHeight > $iMaxH)
							{
								...
								if($showInfo["IMAGE_POPUP"] == "Y") //show in JS window
								{
									...
									<a title="<?echo GetMessage("FILE_ENLARGE")?>" onclick="ImgShw('<?echo CUtil::addslashes($sImagePath)?>','<?echo $intWidth?>','<?echo $intHeight?>', ''); return false;" href="<?echo $sImagePath?>" target="_blank"><img border="0" id="<?echo $showInfo["ID"]?>" src="<?echo $sImagePath?>" width="<?echo intval(roundEx($intWidth/$coeff))?>" height="<?echo intval(roundEx($intHeight/$coeff))?>" /></a>
									</td></tr><?
									$bImageShowed = true;
								}
							}
							...
							{
								?><tr><td class="img-control-image">
								<img id="<?echo $showInfo["ID"]?>" src="<?echo $sImagePath?>" width="<?echo $intWidth?>" height="<?echo $intHeight?>">
								</td></tr><?
								$bImageShowed = true;
							}
						}
					...
					if(!$bImageShowed)
					{
						...
						if($showInfo["PATH"] == "Y")
							echo GetMessage("FILE_TEXT").": ".$sImagePath;
				...
				{
					?><tr><td class="img-control-image"><div style="text-align:left"><?
					echo GetMessage("FILE_NOT_FOUND").": ".$sImagePath;
					?></td></tr><?
				}
			}
	...
	function GetItemViewHTML($itemId)
	{
		...
		if (!$bHandled)
		{
			...
			if ($item['TYPE'] == 'image' || strpos($item['CONTENT_TYPE'], 'image') !== false)
			{
				// It's image
				$arRes = array(
					"html" => "<img src='".$item['PATH']."' width='".$item['WIDTH']."' height='".$item['HEIGHT']."' title='".htmlspecialchars($item['NAME'])."' />",
					"width" => $item['WIDTH'],
					"height" => $item['HEIGHT']
				);
			}
...
class CMedialibItem
{
	...
	function Search($arQuery, $arTypes = array())
	{
		...
		$strSql = "SELECT
					MI.*, MI.*,MCI.COLLECTION_ID, F.HEIGHT, F.WIDTH, F.FILE_SIZE, F.CONTENT_TYPE, F.SUBDIR, F.FILE_NAME,
					".$DB->DateToCharFunction("MI.DATE_UPDATE")." as DATE_UPDATE2
				FROM b_medialib_item MI
		...
		for ($i = 0; $i < $l; $i++)
			$strSql .= " AND MI.SEARCHABLE_CONTENT LIKE '%{".$arQuery[$i]."}%'";
...
class CMedialibTabControl
{
	...
	function Show()
	{
		...
		<table id="<?echo $this->_uniq_id?>" class="imgtab-tabs" cellspacing="0" cellpadding="0"><tr valign="top">

...
_BXPopupWindow.prototype = {
	...
	Show: function (px, py, pNode, name)
	{
		if(!this.pFrame)
			this.Create();
	...
	SetCurStyles: function ()
	{
		...
		var rules, err=false, cssText = '', j, i;
		if(!x1[0].cssRules)
		{
...
{
	var atr = pNode.getAttribute ? pNode.getAttribute("__bxcontainer") : pNode.arAttributes["__bxcontainer"];
	if (atr) 
		return BXUnSerialize(atr);
	return {};
}
...
BXWaitWindow.prototype.Show = function()
{
	if (!this.oDiv)
	{
		this.oDiv = BX("editor_wait_window");
		if (!this.oDiv)
		{
			var oSize = BX.GetWindowInnerSize();
...
BXWaitWindow.prototype.Hide = function()
{
	if (!this.oDiv)
		this.oDiv = BX("editor_wait_window");
		//this.oDiv = BX("editor_wait_window_"+this.pObjName);
...
function BXSetConfiguration(pMainObj, sTarget, method, data)
{
	...
	switch(sTarget)
	{
		...
		case "render_components":
			urladd = "&render_components="+(pMainObj.bRenderComponents ? "Y" : "N");
			break;
		default:
		urladd = "";
		break;
...
function BXGetConfiguration(oCallBack, arParams)
{
	...
	//Toolbars
	pMainObj.RS_toolbars = window.RS_toolbars !== false;
	SETTINGS[pMainObj.name].arToolbarSettings = window.arToolbarSettings;

	//Taskbars
	pMainObj.RS_taskbars = window.RS_taskbars !== false;

...
function BXComponents2Taskbar()
{
	...
	{
		if (!oEl || oEl.getAttribute('__bxtagname') != 'component2')
			return;

		// Run it only when dropped into editor doc
		if (oEl.ownerDocument != oTaskbar.pMainObj.pEditorDocument)
		{
			var newEl = oTaskbar.pMainObj.pEditorDocument.body.appendChild(oEl.cloneNode(false));
			return oTaskbar.OnElementDragEnd(newEl);
		}
		...
		var props = oBXEditorUtils.getCustomNodeParams(oEl);
		oTaskbar.SetParams({id: oEl.id, params: props});
		oEl.removeAttribute('__bxcontainer');
	...
	BXComponents2Taskbar.prototype.OnComp2DblClick = function (e)
	{
		...
		if (pEl && pEl.nodeName)
		{
			if (oTaskbar.pMainObj.pMainObj.bRenderComponents && pEl.getAttribute("__bxtagname") != 'component2')
		...
		var __bxtagname = pEl.getAttribute("__bxtagname");
		if (__bxtagname !== 'component2')
			return;
	...
	BXComponents2Taskbar.prototype._ShowProps = function (_bNew, _pTaskbar, _pElement, bReloadProps)
	{
		...
		else if (window._bx_reload_template_props)
		{
			...
			arProps = oBXEditorUtils.getCustomNodeParams(_pElement);
	...
	BXComponents2Taskbar.prototype.ShowComponentParam = function(P)
	{
		...
		if (!arPropertyParams.PARENT || !arPropertyParams.group_title)
		{
		...
		if (arPropertyParams.PARENT)
		{
			...
			if (_tbls.length > 0)
				...
				tProp =  pParCell.appendChild(BXCreateElement('TABLE', {className : "bxtaskbarprops"}, {}, document));
		}
		else
		{
			tProp = __tProp;
			...
			else
			{
					...
					if (isYes(arPropertyParams.REFRESH))
					{
						window['BX_FD_ONRESULT_' + propertyID] = function(filename, filepath)
						{
							if (typeof filename == 'object')
								...
								oInput.value = filepath + "/" + filename; // From old good file dialog
							fChange();
							_this.ShowProps(_bNew, pPropTsb, _pElement, true);
						}
					}
				...
				else if(arPropertyParams.TYPE == 'COLORPICKER')
				{
					var oCP = pMainObj.CreateCustomElement('BXEdColorPicker',
					{
						...
						function (color) {oInput.value = color.replace(/^#+/g, ""); fChange(); _this.ShowProps(_bNew, pPropTsb, _pElement, true);}
						:
		...
		{
			var oBXHint = new BXHint(oTaskbar.arCurrentTooltips[propertyID]);
			cell.appendChild(oBXHint.oIcon);
			oBXHint.oIcon.style.marginLeft = "5px";
			oBXHint.oIcon.style.marginTop = "2px";
		}
	}
	...
	BXComponents2Taskbar.prototype.BXShowComponent2Panel = function (_bNew, _pTaskbar, _pElement)
	{
		...
		var
			...
			tCompTitle = BXCreateElement('TABLE', {className : "componentTitle"}, {}, document),
		...
		var
			oOption, _el, k, site_template, j,
			__arTemplates = as_arComp2Templates[arAllProps.name],
			_len = __arTemplates.length,
		...
		tProp = row = cell = arPropertyParams = pSelect = oOption = null;
	}
				...
				setTimeout(function ()
					{
						...
						//var arProps = oBXEditorUtils.getCustomNodeParams(calbackParams[2]);
	...
	BXComponents2Taskbar.prototype.UnParseElement = function(node)
	{
		...
		var arAllProps = oTaskbar.GetParams({id: node.arAttributes["id"]});
		var
			//arAllProps = oBXEditorUtils.getCustomNodeParams(node),
			//arAllProps = this.GetParams({id: node.arAttributes["id"]}),
			arPropsVals = arAllProps.paramvals,
		...
		if (arPropsVals)
		{
			...
			for (var i in arPropsVals)
			{
				...
				}catch(e){_alert('ERROR > UnParseElement: '+i+"\n"+arPropsVals[i]);continue;}
			}
...
C2Parser.prototype =
{
	...
	COnChangeView : function()
	{
		...
		else
		{
			...
			if (this.pMainObj.oPropertiesTaskbar)
			{
				oBXEditorUtils.BXRemoveAllChild(this.pMainObj.oPropertiesTaskbar.pCellProps);
	...
	COnDblClick : function (e)
	{
		...
		if (pBlock && pBlock.getAttribute("__bxtagname") == 'component2')
		{
	...
	GetRenderedContent : function(P)
	{
		...
		{
			var content = this.pMainObj.GetContent();
			data += '&source=' + content;
		}
		else if (P.name) // get rendered content of the some component
		{
			...
			// Get params
			if (P.paramvals)
				data += '&' + oBXEditorUtils.ConvertArray2Post(this.C2Taskbar.PrepareParamvals(P.paramvals), 'params');
		}

		req.Action = function(result)
		{
	...
	RenderComponent : function(id, source, bReRender, repEl)
	{
		...
		else
		{
			...
			var pBlock = this.pMainObj.CreateEditorElement("DIV", {id: id, className: 'bxc2-block', __bxtagname: 'component2'});
	...
	{
		var src = pIcon.getAttribute('src');
		if (src)
			pIcon.setAttribute('_src', src);
	...
	StopWaiting : function(id, repEl)
	{
		...
		{
			var _src = pEl.getAttribute('_src');
			if (_src)
				pEl.src = _src;
		}
		return pEl;
	...
	SelectComponent : function(pEl)
	{
		...
		setTimeout(function()
		{
			...
			SETTINGS[obj.pMainObj.name].arTaskbarSettings['BXPropertiesTaskbar'].show = false; // hack
		}, 250);
...
function BXCheckForComponent2(_str, pMainObj, bLCA_mode)
{
	...
	if (_oFunc.name.toUpperCase()=='$APPLICATION->INCLUDECOMPONENT')
	{
		...
		try{
			...
			if (pMainObj.bRenderComponents)
				return '<img id="' + id + '" src="' + c2wait_path + '" _src="' + icon + '" __bxtagname="' + tagname + '" />';
			else
				return '<img id="' + id + '" src="' + icon + '" __bxtagname="' + tagname + '" />';

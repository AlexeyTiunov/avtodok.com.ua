	...
	Search: function()
	{
		...
		var onResult = function(result)
		{
			...
			if (l > 0 && _this.arSearchResult.length == 0)
			{
				...
				_this.oSearchResDialog.DIV.style.height = "400px";
				_this.oSearchResDialog.DIV.style.width = "600px";
				_this.oSearchResDialog.adjustPos();

				_this.oSearchResDialog.PARAMS.buttons[0].enable();
				_this.oSearchResDialog.adjustSize();
			}
		...
		else
		{
			...
			this.oSearchResDialog.SetTitle(FM_MESS.SearchProgress);
			this.oSearchResDialog.PARAMS.buttons[1].enable();
			this.oSearchResDialog.adjustSize();
		}
	...
	Replace: function()
	{
		...
		var onResult = function()
		{
			...
			if (l > 0 && _this.arReplaceResult.length == 0)
			{
				...
				_this.oReplaceResDialog.DIV.style.height = "400px";
				_this.oReplaceResDialog.DIV.style.width = "600px";
				_this.oReplaceResDialog.adjustPos();

				_this.oReplaceResDialog.PARAMS.buttons[0].enable();
				_this.oReplaceResDialog.adjustSize();
			}
		...
		else
		{
			...
			_this.oReplaceResDialog.DIV.style.height = "120px";
			...
			this.oReplaceResDialog.adjustSize();
		}
...
BXFMCopy.prototype = {
	...
	ShowAskUserDialog: function(Params)
	{
		...
		if (!this.oAskUserDialog)
		{
			...
			setTimeout(function()
			{
				_this.oAskUserDialog.PARTS.CONTENT.appendChild(BX('bx_copy_ask_dialog'));
				BX('bx_copy_ask_dialog').style.display = "block";

				_this.pAskToAllCont = _this.oAskUserDialog.PARTS.FOOT.appendChild(BX.create("DIV", {props: {className: "bx-copy-to-all" }, html: "<table><tr><td><input type='checkbox' id='bx_copy_ask_to_all'></td><td><label  for='bx_copy_ask_to_all'>" + FM_MESS.ToAll + "</label></td></tr></table>"}));

				_this.oAskUserDialog.PARTS.CONTENT.style.height = BX.browser.IsIE() ? "160px" : "170px";
			}, 50);

...
BXMediaLib.prototype =
{
		...
		this.Request(
		{
			...
			handler: function(innerHTML)
			{
				...
				if (innerHTML)
				{
					...
					jsFloatDiv.Show(_this.pWnd, left, top);
	...
	Submit: function()
	{
		...
		var
			...
			resTupe = this.oConfig.resType,
			oRes = this.oConfig.arResultDest;

		if (resTupe == "FUNCTION" && typeof window[oRes.FUNCTION_NAME] == 'function')
		{
			...
			handler: function()
			{
				if (window.bx_req_res !== false)
				{
					...
					var oCol =
					{
						...
						name: bxhtmlspecialchars(postData.name),
						desc: bxhtmlspecialchars(postData.desc),
						date: '',
						keywords: bxhtmlspecialchars(postData.keywords),
					...
					else
					{
						...
						pTitle.childNodes[1].innerHTML = oCol.name;
						pTitle.title = bxspcharsback(oCol.desc || oCol.name);
					}

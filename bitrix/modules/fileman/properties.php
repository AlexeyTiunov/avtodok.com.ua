...
<?
	...
	function GetPropertyFieldHtml($arProperty, $value, $strHTMLControlName)
	{
		...
		if ($arProperty['MULTIPLE'] == 'Y' && isset($GLOBALS['YANDEX_MAP_PROPERTY_'.$arProperty['ID']]))
		{
			// property is multimple and map is already showed

			$MAP_ID = $GLOBALS['YANDEX_MAP_PROPERTY_'.$arProperty['ID']];
		}
		else
		{
			$MAP_ID = 'map_yandex_'.$arProperty['CODE'].'_'.$arProperty['ID'];
			$GLOBALS['YANDEX_MAP_PROPERTY_'.$arProperty['ID']] = $MAP_ID;
	...
	function BaseConvertToDB($value)
	{
		...
		if ($value["B_NEW_FILE"] != "N") // New video or replacing old
		{
			if (strlen($value["CUR_PATH"]) > 0 && $value["DEL_CUR_FILE"] == "Y" && CIBlockPropertyVideo::CheckFileInUploadDir($value["CUR_PATH"]))
			{
				...
				if(@unlink($cur_path_))
				{
					...
					if(COption::GetOptionInt("main", "disk_space") > 0)
						CDiskQuota::updateDiskQuota("file", filesize($cur_path_), "delete");
				}
			}
			...
			else if (isset($value["FILE"]) && strlen($value["FILE"]["tmp_name"]) > 0)
			{
				...
				if (!is_dir($_SERVER["DOCUMENT_ROOT"].$pathToDir))
				...
				if(strlen($ext) > 0 && !in_array($ext, CFileMan::GetScriptFileExt()) && !substr($name, 0, 1) != ".")
				{
					...
					while(file_exists($_SERVER["DOCUMENT_ROOT"].Rel2Abs($pathToDir, $name)))
						$name = $baseNamePart."_(".++$ind.").".$ext; // 3. Rename

					$pathto = Rel2Abs($pathToDir, $name);
					if (is_uploaded_file($value["FILE"]["tmp_name"]) && copy($value["FILE"]["tmp_name"], $_SERVER["DOCUMENT_ROOT"].$pathto))
					{

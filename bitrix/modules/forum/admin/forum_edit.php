...
/********************************************************************
				Action
********************************************************************/
if ($REQUEST_METHOD == "POST" && $forumPermissions >= "W" && strlen($Update) > 0 && check_bitrix_sessid())
{
	...
	else
	{
		...
		if ($ID > 0)
		{
			$res = CForumNew::Update($ID, $arFields);
		}
		else
		{
...
?>
	<tr>
		<td width="50%"><?=GetMessage("ACTIVE")?>:</td>
		<td width="50%">
	...
	<tr>
		...
		<td valign="top">
			<textarea name="DESCRIPTION" rows="3" cols="30"><?=$arForum["DESCRIPTION"]; ?></textarea>
		</td>
	</tr>

	<tr class="heading">
		<td colspan="2"><span class="required">*</span><?=GetMessage("FE_SITES_PATHS")?><input type="hidden" name="SITES" /></td>
	</tr>
	<?
	foreach ($arSites as $key => $res)
	{
		...
		<tr>
			...
			<td>
				<input type="text" name="SITE_PATH[<?=$res["LID"]?>]" size="40" maxlength="255" value="<?if (array_key_exists($res["LID"], $arForum["SITES"])) echo $arForum["SITES"][$res["LID"]]?>">
			</td>
		</tr>
		<?
	}
...
?>
	<tr>
		<td width="50%"><?=GetMessage("INDEX")?>:</td>
		<td width="50%">

...
while ($arRes = $rsData->NavNext(true, "t_"))
{
	...
	$row->AddViewField("USER_ID", "<a href='user_edit.php?lang=".LANGUAGE_ID."&ID=".$t_USER_ID."' title='".GetMessage("MAIN_EDIT_TITLE")."'>".$t_USER_ID."</a>");
...
/*******************************************************************/
	...
	<tr valign="center">
		...
		<select name="FilterType">
			<option value="login"<?if($find_type=="login") echo " selected"?>><?=GetMessage('FM_FLT_LOGIN')?></option>
			<option value="email"<?if($find_type=="email") echo " selected"?>><?=GetMessage('FM_FLT_EMAIL')?></option>
			<option value="name"<?if($find_type=="name") echo " selected"?>><?=GetMessage('FM_FLT_FIO')?></option>
		</select>
		</td>
	</tr>
	<tr valign="center">

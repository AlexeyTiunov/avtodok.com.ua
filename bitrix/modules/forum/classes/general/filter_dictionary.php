...
class CAllFilterUnquotableWords
{
	function CheckPattern($sPattern, &$sError)
	{
		$arError = array();
		// Check Modificators
		$sModificators = strrchr(substr($sPattern, 1) , substr($sPattern, 0, 1));
		...
		while ($lett = $letters->Fetch())
		{
			...
			for ($ii = 0; $ii < count($arrRepl); $ii++)
			{
				...
				if (strLen($lett["LETTER"])==1)
				{
					...
					elseif (substr($arrRepl[$ii], 0, 1) == "(" && substr($arrRepl[$ii], -1, 1) == ")")
						$arrRes[$ii] = $arrRepl[$ii];
...
//					$lettPatt[$lett["LETTER"]] = str_replace("+", "*", $lettPatt[$lett["LETTER"]]);
			...
			{
				$lettersPatt["/".$lett["LETTER"]."/is".BX_UTF_PCRE_MODIFIER] = "(".implode("|", $arrRes).")";
			}
		}
		// letters 

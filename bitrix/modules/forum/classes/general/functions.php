...
class textParser
{
	...
	function textParser($strLang = False, $pathToSmile = false)
	{
		...
		if ($strLang === False) 
			$strLang = LANGUAGE_ID;
		if ($pathToSmile === false)
			$pathToSmile = "/bitrix/images/forum/smile/"; 
	...
	function convert($text, $allow = array("HTML" => "N", "ANCHOR" => "Y", "BIU" => "Y", "IMG" => "Y", "QUOTE" => "Y", "CODE" => "Y", "FONT" => "Y", "LIST" => "Y", "SMILES" => "Y", "NL2BR" => "N", "VIDEO" => "Y"), $type = "html")	//, "KEEP_AMP" => "N"
	{
		...
		if ($allow["HTML"] != "Y")
		{
			...
			{
				$text = preg_replace(
					"#<img[^>]+src\s*=[\s\013\014]*(((http|https|ftp)://[.-_:a-z0-9@]+)*(\/[-_/=:.a-z0-9@{}&?]+)+)[\s\013\014]*[^>]*>#is".BX_UTF_PCRE_MODIFIER, 
					"[img]\\1[/img]", $text);
			}
			if ($allow["QUOTE"]=="Y")
			{
		...
		if ($allow["SMILES"]=="Y")
		{
			if (count($this->smiles) > 0)
			{
				...
				{
					$code  = str_replace(array(chr(34), chr(39)), array("\013", "\014"), $row["TYPING"]);
					$image = str_replace(array(chr(34), chr(39)), array("\013", "\014"), $row["IMAGE"]);
...
//		$text = str_replace(array("  ", "\t", ), array("&nbsp;&nbsp;", "&nbsp;&nbsp;&nbsp;"), $text);
	...
	function convert_image_tag($url = "", $type = "html")
	{
		...
		if (strpos($url, "/bitrix/components/bitrix/forum.interface/show_file.php?fid=") === false)
		{
			...
			if (!$bErrorIMG && !preg_match("/^((http|https|ftp)\:\/\/[-_:.a-z0-9@]+)*(\/[-_+\/=:.a-z0-9@%\013]+)$/i".BX_UTF_PCRE_MODIFIER, $url)) $bErrorIMG = True;
		}
		if ($bErrorIMG)
			return "[img]".$url."[/img]";
		if ($type != "html")

...
/***************** /Event ******************************************/
	...
	function SendMailMessage($MID, $arFields = array(), $strLang = false, $mailTemplate = false)
	{
		...
		//Если это сообщение из соцсети: найдем, есть ли почтовая обработка для данной соцсети
		if($arTopic["SOCNET_GROUP_ID"]>0)
		{
			if(CModule::IncludeModule("mail") && CModule::IncludeModule("socialnetwork"))
			{
				...
				if($arMailParams)
				{
					...
					if($arMessage["XML_ID"]=='')
					{
						//проверим наличие поля MSG_ID, если нету - сгенерируем
					...
					//выберем MSG_ID темы, это будет IN_REPLY_TO
					if($arTopic["XML_ID"]=='')
					{
					...
					//заполним FROM_EMAIL из AUTHOR_NAME + FROM_EMAIL или AUTHOR_EMAIL или адрес из b_user по AUTHOR_ID в зависимости от настроек почтовой обработки
					...
					//заполним REPLY_TO из настроек обработчика писем

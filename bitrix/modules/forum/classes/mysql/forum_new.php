...
/***************** /Event ******************************************/
	...
	function OnReindex($NS = array(), $oCallback = NULL, $callback_method = "")
	{
		...
		$strSql = 
			"SELECT FT.ID as TID, FM.ID as MID, FM.ID AS ID, FT.FORUM_ID, FT.TITLE, FT.DESCRIPTION, FT.TAGS, FT.HTML as FT_HTML, 
				FM.PARAM1, FM.PARAM2, FM.POST_MESSAGE, FM.POST_MESSAGE_FILTER, FM.POST_MESSAGE_HTML, FM.AUTHOR_NAME, FM.NEW_TOPIC, 
				".$DB->DateToCharFunction("FM.POST_DATE")." as POST_DATE, FT.SOCNET_GROUP_ID, FT.OWNER_ID
			FROM b_forum_message FM, b_forum_topic FT, b_forum F 
			".$strNSJoin."

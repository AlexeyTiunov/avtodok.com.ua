...
//			"PREPARE_RESULT_FUNC"	=>	Array("CForumEMail", "SocnetPrepareVars"),
	...
	function SocnetEMailMessageAdd($arMessageFields, $ACTION_VARS)
	{
		...
		while ($arAttach = $rsAttach->Fetch())
		{
			...
			$dir = $_SERVER["DOCUMENT_ROOT"]."/".COption::GetOptionString("main", "upload_dir", "upload")."/tmp/forumemail/";
			CheckDirPath($dir);
			$filename = $dir.md5(uniqid("")).".tmp";
			if($handle = @fopen($filename, 'wb'))
			{
				if(!@fwrite($handle, $arAttach["FILE_DATA"])) 
					fclose($handle);
				else
				{
					fclose($handle);
					...
					$arFiles = array($arFile); $arFilter = array("FORUM_ID" => $FORUM_ID);
		...
		*/
		...
		foreach($arrUnlink as $unlink_file)
			@unlink($unlink_file);
		...
		if($MESSAGE_ID>0)
		{
			...
			CSocNetLog::Add(array(
				"ENTITY_TYPE" => SONET_ENTITY_GROUP,
				"ENTITY_ID" => $SOCNET_GROUP_ID,
				"EVENT_ID" => "forum",
				"=LOG_DATE" => $GLOBALS["DB"]->CurrentTimeFunction(),
				"TITLE_TEMPLATE" => ($arFields["NEW_TOPIC"] == 'Y'
					? str_replace("#AUTHOR_NAME#", $AUTHOR_NAME, CForumEmail::GetLangMessage("FORUM_MAIL_SOCNET_TITLE_TOPIC", $lang))
					: str_replace("#AUTHOR_NAME#", $AUTHOR_NAME, CForumEmail::GetLangMessage("FORUM_MAIL_SOCNET_TITLE_MESSAGE", $lang))
					),
				"TITLE" => $subject,

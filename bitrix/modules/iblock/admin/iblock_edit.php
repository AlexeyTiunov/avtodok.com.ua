...
if($Perm>="X" && $REQUEST_METHOD=="POST" && strlen($_POST["Update"])>0 && !isset($_POST["propedit"]) && check_bitrix_sessid())
{
	...
	if(!$bVarsFromForm)
	{
		...
		else
		{
			...
			if(!$bVarsFromForm)
			{
				if($bBizproc && $_REQUEST['BIZ_PROC_ADD_DEFAULT_TEMPLATES']=='Y' && CBPDocument::GetNumberOfWorkflowTemplatesForDocumentType(array("iblock", "CIBlockDocument", "iblock_".$ID))<=0)

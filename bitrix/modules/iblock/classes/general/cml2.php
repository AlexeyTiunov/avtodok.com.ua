	...
	function ImportElement($arXMLElement, &$counter, $bWF, $arParent)
	{
		...
		else
		{
			...
			if(isset($arXMLElement[GetMessage("IBLOCK_XML2_PICTURE")]))
			{
				...
				{
					$i = 1;
					while(isset($arXMLElement[GetMessage("IBLOCK_XML2_PICTURE").$i]))
					{
						$arElement["PROPERTY_VALUES"][$prop_id]["n".$i] = $this->ResizePicture($arXMLElement[GetMessage("IBLOCK_XML2_PICTURE").$i], $this->detail);
						$i++;
					}

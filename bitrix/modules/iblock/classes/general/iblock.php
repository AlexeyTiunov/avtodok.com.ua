...
class CAllIBlock
{
	...
	function GetPanelButtons($IBLOCK_ID=0, $ELEMENT_ID=0, $SECTION_ID=0, $arOptions=array())
	{
		...
		if(array_key_exists("RETURN_URL", $arOptions) && strlen($arOptions["RETURN_URL"]) > 0)
			$return_url = $arOptions["RETURN_URL"];
		elseif(defined("BX_AJAX_PARAM_ID"))
		...
		{
			$url = "/bitrix/admin/iblock_element_edit.php?type=".$arIBlock["IBLOCK_TYPE_ID"]."&lang=".LANGUAGE_ID."&IBLOCK_ID=".$IBLOCK_ID."&filter_section=".$SECTION_ID."&IBLOCK_SECTION_ID=".$SECTION_ID."&return_url=".UrlEncode($return_url);

			$action = $APPLICATION->GetPopupLink(array("URL" => $url."&bxpublic=Y&from_module=iblock", "PARAMS" => array("width" => 700, 'height' => 500, 'resize' => false)));
			$arButton = array(
			...
			$arButton["ACTION"] = "javascript:jsUtils.Redirect(arguments, '".CUtil::JSEscape($url)."')";
			$arButtons["submenu"]["add_element"] = $arButton;

			if($ELEMENT_ID > 0)
			{
				$url = "/bitrix/admin/iblock_element_edit.php?type=".$arIBlock["IBLOCK_TYPE_ID"].$s."&lang=".LANGUAGE_ID."&IBLOCK_ID=".$IBLOCK_ID."&ID=".$ELEMENT_ID."&filter_section=".$SECTION_ID."&return_url=".UrlEncode($return_url);
				...
				if($bWorkflow)
				{
					$url = "/bitrix/admin/iblock_history_list.php?type=".$arIBlock["IBLOCK_TYPE_ID"]."&lang=".LANGUAGE_ID."&IBLOCK_ID=".$IBLOCK_ID."&ELEMENT_ID=".$ELEMENT_ID."&filter_section=".$SECTION_ID."&return_url=".UrlEncode($return_url);
					$arButton = array(
						"TEXT" => GetMessage("IBLOCK_PANEL_HISTORY_BUTTON"),
						"TITLE" => GetMessage("IBLOCK_PANEL_HISTORY_BUTTON"),
						"ACTION" => "javascript:jsUtils.Redirect(arguments, '".CUtil::JSEscape($url)."')",
			...
			if( ($iblock_permission > "U") && ($ELEMENT_ID <= 0) )
			{
				...
				if(($arIBTYPE = $rsIBTYPE->Fetch()) && ($arIBTYPE["SECTIONS"] == "Y"))
				{
					if($SECTION_ID > 0)
					{
						...
						$url = "/bitrix/admin/iblock_section_edit.php?ID=". $SECTION_ID."&type=".$arIBlock["IBLOCK_TYPE_ID"]."&lang=".LANGUAGE_ID. "&IBLOCK_ID=". $IBLOCK_ID."&filter_section=".$SECTION_ID."&return_url=".UrlEncode($return_url);
					...
					$url = "/bitrix/admin/iblock_section_edit.php?type=".$arIBlock["IBLOCK_TYPE_ID"]."&lang=".LANGUAGE_ID."&IBLOCK_ID=".$IBLOCK_ID."&IBLOCK_SECTION_ID=".$SECTION_ID."&filter_section=".$SECTION_ID."&return_url=".UrlEncode($return_url);
		...
		if( ($IBLOCK_ID > 0) && ($iblock_permission >= $max_permission) )
		{
			...
			{
				$url = "/bitrix/admin/iblock_edit.php?type=".$arIBlock["IBLOCK_TYPE_ID"]."&lang=".LANGUAGE_ID."&ID=".$IBLOCK_ID."&return_url=".UrlEncode($return_url);
				$arButton = array(
...
**************/
	...
	function _MergeIBArrays($iblock_id, $iblock_code, $iblock_id2 = false, $iblock_code2 = false)
	{

	...
	function MkFilter($arFilter, &$arJoinProps, &$arAddWhereFields, $level = 0)
	{
		...
		foreach($arFilter as $orig_key => $val)
		{
			...
			case "CHECK_BP_PERMISSIONS":
				if(IsModuleInstalled('bizproc') && !$USER->IsAdmin())
				{
			...
			case "CHECK_BP_TASKS_PERMISSIONS":
				if(IsModuleInstalled('bizproc') && CModule::IncludeModule("socialnetwork") && !$USER->IsAdmin())
				{
					$val = explode("_", $val);
...
//							$arUserGroups[] = SONET_RELATIONS_TYPE_FRIENDS2;
						...
						WHERE
							...
							AND (
								...
								OR (P.OBJECT_ID = 'responsible' AND ".$userId." IN (
									...
									WHERE SFP0.CODE='TASKASSIGNEDTO'
		...
		if($arFilter["SHOW_HISTORY"] != "Y")
			$arSqlSearch[] = "((BE.WF_STATUS_ID=1 AND BE.WF_PARENT_ELEMENT_ID IS NULL)".($arFilter["SHOW_NEW"]=="Y"? " OR (BE.WF_NEW='Y'".$SHOW_BP_NEW.")": "").")";
		...
		if($IBLOCK_CACHE[$IBLOCK_ID])
		{
			...
			if(is_array($ar))
			{
				if(
					(
						intval($arFields["WF_PARENT_ELEMENT_ID"]) == 0
						|| intval($arFields["WF_PARENT_ELEMENT_ID"]) == intval($ID)
					)
	...
	function __GetFileContent($FILE_ID)
	{
		...
		if($arFile)
		{
			...
			{
				if($arrFile = ExecuteModuleEventEx($arEvent, array($_SERVER["DOCUMENT_ROOT"]."/".$arFile["SRC"])))
					break;
			}

...
class CAllIBlockSection
{
	...
	function Add($arFields, $bResort=true, $bUpdateSearch=true, $bResizePictures=false)
	{
		...
		elseif($IBLOCK_ID && !$GLOBALS["USER_FIELD_MANAGER"]->CheckFields("IBLOCK_".$IBLOCK_ID."_SECTION", $ID, $arFields))
		{
			$Result = false;
		}
		else
	...
	function Update($ID, $arFields, $bResort=true, $bUpdateSearch=true, $bResizePictures=false)
	{
		...
		$db_record = CIBlockSection::GetList(Array(), Array("ID"=>$ID, "CHECK_PERMISSIONS"=>$arFields["CHECK_PERMISSIONS"]));
		...
		if(strlen($this->LAST_ERROR)<=0)
		{
			...
			if(IntVal($arFields["IBLOCK_SECTION_ID"])>0)
			{
				...
				if($ar = $r->Fetch())
				{
					...
					{
						//Check if parent section iblock is the same for new section added
						if($ar["IBLOCK_ID"] != $arFields["IBLOCK_ID"])
						{
							$this->LAST_ERROR .= GetMessage("IBLOCK_BAD_BLOCK_SECTION_ID_PARENT")."<br>";
						}
					}
			...
			if($arFields["IBLOCK_ID"])
				$IBLOCK_ID = $arFields["IBLOCK_ID"]; //for new section
			elseif(is_array($arthis))
				$IBLOCK_ID = $arthis["IBLOCK_ID"]; //for section with parent
			else
			{
				$rthis = CIBlockSection::GetByID($ID); //for root section
				if($arthis = $rthis->Fetch())
					$IBLOCK_ID = $arthis["IBLOCK_ID"];
			}

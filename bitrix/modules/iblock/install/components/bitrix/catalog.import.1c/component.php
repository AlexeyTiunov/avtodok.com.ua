...
else
{
	...
	$DIR_NAME = "/".COption::GetOptionString("main", "upload_dir")."/1c_catalog";
	$ABS_FILE_NAME = false;
	$WORK_DIR_NAME = false;
	...
	elseif(($_GET["mode"] == "import") && $ABS_FILE_NAME)
	{
		...
		elseif($NS["STEP"] == 4)
		{
			...
			$obCatalog->Init($NS, $WORK_DIR_NAME, $arParams["USE_CRC"], $preview, $detail, $arParams["USE_OFFERS"], $arParams["USE_IBLOCK_TYPE_ID"]);
		...
		elseif($NS["STEP"] == 7)
		{
			...
			{
				$rs = $DB->Query("select count(*) C from b_xml_tree where PARENT_ID = ".$NS["XML_ELEMENTS_PARENT"]);
				$ar = $rs->Fetch();
				$NS["DONE"]["ALL"] = $ar["C"];
			}

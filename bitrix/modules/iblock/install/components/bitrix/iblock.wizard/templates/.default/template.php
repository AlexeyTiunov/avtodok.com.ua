...
<tr>
	...
	<td>
		...
		<? if (count($arResult['SECTIONS'])) { ?>
			...
			{
				$img = $templateFolder.'/images/'.LANG.'/button_back.gif';
			...
			elseif ($arParams['BACK_URL']) 
			{
				$img = $templateFolder.'/images/'.LANG.'/button_back.gif';
				if (file_exists($_SERVER['DOCUMENT_ROOT'].$img))
					echo '<input type=image src="'.$img.'" onclick="javascript:window.location=\''.$arParams['BACK_URL'].'\';return false;">';
				else
					echo '<input type=submit value="'.GetMessage('WZ_BTN_BACK').'" name="back">';
			} 
		?>
		...
		<?
			$img = $templateFolder.'/images/'.LANG.'/button_next.gif';
			if (file_exists($_SERVER['DOCUMENT_ROOT'].$img))
		...
		<? } else { // Finish ?>
			...
			<img src="/bitrix/images/1.gif" width=1 height=1>
			<?
			$img = $templateFolder.'/images/'.LANG.'/button_done.gif';
			if (file_exists($_SERVER['DOCUMENT_ROOT'].$img))
				echo '<input type=image src="'.$img.'" name="wizard" onclick="Goto(\''.$arParams['NEXT_URL'].'\')">';
			else
				echo '<input type=submit value="'.GetMessage('WZ_BTN_FINISH').'" name="wizard" onclick="Goto(\''.$arParams['NEXT_URL'].'\')">';
			?>
		<? } ?>
		</p>
	</td>

...
if (!defined("UPDATE_SYSTEM_VERSION"))
	define("UPDATE_SYSTEM_VERSION", "9.1.2");
...
if ($DB->type == "MSSQL")
{
	...
	if ($arQueryRes = $dbQueryRes->Fetch())
	{
		...
		if (IntVal($arCurMSSqlVer[0]) < 9
			|| IntVal($arCurMSSqlVer[0]) == 9 && IntVal($arCurMSSqlVer[1]) < 0
			|| IntVal($arCurMSSqlVer[0]) == 9 && IntVal($arCurMSSqlVer[1]) == 0 && IntVal($arCurMSSqlVer[2]) < 0)
		{
	...
	<tr>
		<td colspan="2">
			...
			if ($arUpdateList)
			{
				...
				$newLicenceSigned = COption::GetOptionString("main", "new_license7_sign", "N");
	...
	<tr>
		<td colspan="2">
			...
			<table border="0" cellspacing="1" cellpadding="3">
				...
				<?if (array_key_exists("CLIENT", $arUpdateList)):?>
					<tr>
				...
				<tr>
					...
					<td><?echo ($USER->CanDoOperation('edit_other_settings')? CUpdateClient::GetLicenseKey() : "XXX-XX-XXXXXXXXXXX")?></td>
				</tr>
				<?if (array_key_exists("CLIENT", $arUpdateList)):?>
					<tr>

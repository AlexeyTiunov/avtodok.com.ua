...
class CAllAgent
{
		...
		)
	{
		...
		$strSql = "SELECT 'x' FROM b_agent WHERE NAME='".$DB->ForSql($name,255)."' AND USER_ID".($user_id ? "=".intval($user_id) : " IS NULL");
	...
	function RemoveAgent($name, $module="", $user_id=false)
	{
		...
		$strSql = "DELETE FROM b_agent WHERE NAME='".$DB->ForSql($name,255)."' ".$module." AND  USER_ID".($user_id ? "=".intval($user_id) : " IS NULL");

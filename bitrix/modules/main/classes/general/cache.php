...
class CCacheManager
{
	...
	{
		$this->InitCompSalt();
		$path = "/".SITE_ID.$relativePath.$this->SALT;
		return $path;
	}
	...
	function EndTagCache()
	{
		...
		if($this->bWasTagged)
		{
			...
			{
				$arFields["RELATIVE_PATH"] = "'".$DB->ForSQL($arCompCache[0], 255)."'";
				$path = $arCompCache[0];
				...
				if(count($arAllTags))
				{
					$sql = "
						...
						VALUES (".$arFields["SITE_ID"].", ".$arFields["CACHE_SALT"].", ".$arFields["RELATIVE_PATH"].",";

					foreach($arAllTags as $tag => $t)
					{
						$arFields["TAG"] = "'".$DB->ForSQL($tag, 50)."'";

						if(!isset($this->DBCacheTags[$path]) || !isset($this->DBCacheTags[$path][$tag]))
						{
							$DB->Query($sql." ".$arFields["TAG"].")");
							$this->DBCacheTags[$path][$tag]++;
						}
					}
				}
			}

...
class CControllerTools
{
	function PackFileArchive($path)
	{
		...
		if (file_exists($path))
		{
			...
			CheckDirPath($_SERVER['DOCUMENT_ROOT'] . $arcname);
	...
	function UnpackFileArchive($strfile, $path_to)
	{
		...
		include_once(dirname(__FILE__) . '/tar_gz.php');
		$arcname = '/bitrix/tmp/controller/' . md5(uniqid(rand(), true)) . '.tar.gz';
		CheckDirPath($_SERVER['DOCUMENT_ROOT'] . $arcname);
		...
		if ($fp = fopen($_SERVER['DOCUMENT_ROOT'] . $arcname, 'wb'))
		{
			fwrite($fp, $strfile);
			fclose($fp);
			...
			unlink($_SERVER['DOCUMENT_ROOT'] . $arcname);
		}

...
class CAllEvent
{
	function SendImmediate($event, $lid, $arFields, $Duplicate = "Y", $message_id="")
	{
		...
		CEvent::HandleEvent($arLocalFields);
	}
	...
	function Send($event, $lid, $arFields, $Duplicate = "Y", $message_id="")
	{
		...
		$fname = $_SERVER["DOCUMENT_ROOT"].BX_PERSONAL_ROOT."/managed_cache/".$DB->type."/b_event";
		if(CACHED_b_event!==false && file_exists($fname))
			unlink($fname);
	...
	function HandleEvent($arEvent)
	{
		...
		while($db_mail_result_array = $db_mail_result->Fetch())
		{
			...
			if(COption::GetOptionString("main", "convert_mail_header", "Y")=="Y")
			{
				...
				$subject = CAllEvent::EncodeSubject($subject, $charset);
			}
			...
			$message = CAllEvent::ReplaceTemplate($db_mail_result_array["MESSAGE"], $arFields);
		...
		if($arResult["Was"])
		{
			...
			{
				if($arResult["Fail"])
					$flag = "P"; // частично отправлены
				else
					$flag = "Y"; // все отправлены
			}
			else
			{
				if($arResult["Fail"])
					$flag = "F"; // ни по одному из шаблонов не было успешной отправки письма
			}
		}

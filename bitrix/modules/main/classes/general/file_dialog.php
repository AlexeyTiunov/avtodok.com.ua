...
<?
	...
	function Start($Params)
	{
		...
		{
			$armt = CAdminFileDialog::GetMenuTypes($Params['site'], $Params['path']);
	...
	function LoadItems($Params)
	{
		...
		echo '<script>';
		if ($Params['bAddToMenu'])
			CAdminFileDialog::GetMenuTypes($Params['site'], $Params['path'], true);

		if ($Params['loadRecursively'])
			CAdminFileDialog::GetItemsRecursively(array('path' => $Params['path'], 'site' => $Params['site'], 'bCheckEmpty' => true, 'getFiles' => $Params['getFiles'], 'loadRoot' => $Params['loadRoot'], 'bThrowException' => true));
		else
			CAdminFileDialog::GetItems(array('path' => $Params['path'], 'site' => $Params['site'], 'bCheckEmpty' => true, 'getFiles' => $Params['getFiles']));
...
<?
		...
		CAdminFileDialog::GetItemsRecursively(array('path' => $Params['path'], 'site' => $Params['site'], 'bCheckEmpty' => true, 'getFiles' => $Params['getFiles'], 'loadRoot' => true, 'bFindCorrectPath' => true, 'bThrowException' => false));
...
<?
	...
	function MakeNewDir($Params)
	{
		if(CModule::IncludeModule("fileman"))
		{
			...
			{
				if (strlen($dirname) > 0 && ($mess = CAdminFileDialog::CheckFileName($dirname)) !== true)
					$strWarning = $mess;
		...
		if ($strWarning == '')
			CAdminFileDialog::LoadItems(array('path' => $path, 'site' => $site, 'bAddToMenu' => $Params['bAddToMenu'], 'loadRecursively' => false, 'getFiles' => $Params['getFiles']));
	}
	...
	function Remove($Params)
	{
		...
		if ($strWarning == '')
		{
			...
			CAdminFileDialog::LoadItems(array('path' => $parPath, 'site' => $site, 'bAddToMenu' => $Params['bAddToMenu'], 'loadRecursively' => false, 'getFiles' => $Params['getFiles']));
		}
	}
	...
	function Rename($Params)
	{
		if(CModule::IncludeModule("fileman"))
		{
			...
			{
				if (strlen($name) > 0 && ($mess = CAdminFileDialog::CheckFileName($name)) !== true)
					$strWarning = $mess;
...
<?
	...
	function ShowUploadForm($Params)
	{
		...
		$res = $Params['file'] ? CAdminFileDialog::UploadFile($Params) : '';
		?>
<HTML>
<HEAD><?=$res?></HEAD>
...
<?
	...
	function UploadFile($Params)
	{
		...
		if (isset($F["tmp_name"]) && strlen($F["tmp_name"]) > 0 && strlen($F["name"]) > 0 || is_uploaded_file($F["tmp_name"]))
		{
			...
			if (strlen($filename) > 0 && ($mess = CAdminFileDialog::CheckFileName($filename)) !== true)
				$strWarning = $mess;

...
class CAllMain
{
	...
	function DrawIcons($arIcons, $arParams=array())
	{
		...
		$res .= '
			var arItems_'.$areaId.' = '.CUtil::PhpToJSObject($arJSIcons).';
			var obMenu_'.$areaId.';
			if(window.PopupMenu)
				obMenu_'.$areaId.' = new PopupMenu(\'menu_'.$areaId.'\');
		</script>';
...
var oBXHint'.$areaId.';
if(window.BXHint)
	oBXHint'.$areaId.' = new BXHint(\''.CUtil::JSEscape($arParams["TOOLTIP"]).'\', document.getElementById("bx_incl_area_panel_'.$areaId.'"), {width: false});
</script>';
		}
		$res .= '</div>';
		...
		if(is_array($arIcons))
		{
			foreach($arIcons as $arIcon)
			{
				if(isset($arIcon['DEFAULT']) && $arIcon['DEFAULT'] == true)
				{
					...
					if(bx_incl_area && window.jsPopup)
					{
	...
	function IncludeComponent($componentName, $componentTemplate, $arParams = array(), $parentComponent = null, $arFunctionParams = array())
	{
		...
		$sSrcFile = "";
		$iSrcLine = 0;
		if((!isset($arFunctionParams["HIDE_ICONS"]) || $arFunctionParams["HIDE_ICONS"] <> "Y")
		...
		if($bDrawIcons)
		{
					...
					if(strlen($template->GetSiteTemplate()) > 0)
					{
						...
						$arIcons[] = array(
							'URL' => 'javascript:'.$APPLICATION->GetPopupLink(array(
									'URL' => "/bitrix/admin/public_file_edit_src.php?bxpublic=Y&site=".SITE_ID."&".'path='.urlencode($template->GetFile())."&back_url=".urlencode($_SERVER["REQUEST_URI"])."&lang=".LANGUAGE_ID,
									'PARAMS' => array(
										'width' => 770,
										'height' => 570,
										'resize' => true
						...
						if(StrLen($template->GetFolder()) > 0)
						{
							if(file_exists($_SERVER["DOCUMENT_ROOT"].$template->GetFolder()."/style.css"))
							{
								...
								$arIcons[] = array(
									'URL' => 'javascript:'.$APPLICATION->GetPopupLink(array(
											'URL' => "/bitrix/admin/public_file_edit_src.php?bxpublic=Y&site=".SITE_ID."&".'path='.urlencode($template->GetFolder()."/style.css")."&back_url=".urlencode($_SERVER["REQUEST_URI"])."&lang=".LANGUAGE_ID,
											'PARAMS' => array(
												'width' => 770,
												'height' => 570,
												'resize' => true
			...
			if($arComponentDescription && is_array($arComponentDescription))
			{
				...
				if(array_key_exists("CACHE_PATH", $arComponentDescription) && $showMode == "configure" && $USER->CanDoOperation('cache_control'))
				{
					...
					{
						$arIcons[] = array(
							"URL" => "javascript:jsComponentUtils.ClearCache('component_name=".urlencode(CUtil::addslashes($componentName))."&site_id=".SITE_ID."&".bitrix_sessid_get()."');",
							"ICON" => "del-cache",
			...
			if($bComponentAccess && !$parentComponent && $bSrcFound && $showMode == "configure")
			{
				if(!empty($arIcons))
					$arIcons[] = array("SEPARATOR"=>true);
				$arIcons[] = array(
					"URL" => "javascript:jsComponentUtils.EnableComponent('component_name=".urlencode(CUtil::addslashes($componentName)). //$rel_path
						"&lang=".urlencode(CUtil::addslashes(LANGUAGE_ID)).
						"&src_path=".urlencode(CUtil::addslashes($sSrcFile)).
						"&src_line=".$iSrcLine.
						"&src_site=".urlencode(CUtil::addslashes(SITE_ID)).
						'&active='.($bComponentEnabled? 'N':'Y').
						'&'.bitrix_sessid_get()."');",
					"TITLE" => ($bComponentEnabled? GetMessage("main_comp_disable"):GetMessage("main_comp_enable")),
					"ALT" => ($bComponentEnabled? GetMessage("main_comp_disable_title"):GetMessage("main_comp_enable_title")),
				);
			}
	...
	function IncludeFile($rel_path, $arParams = Array(), $arFunctionParams = Array())
	{
		...
		if($bDrawIcons)
		{
			...
			else
			{
				...
				{
					case 'html':
						$editor = '/bitrix/admin/public_file_edit.php?bxpublic=Y&from=includefile&';
						$resize = 'false';
					break;
					...
					case 'php':
						$editor = '/bitrix/admin/public_file_edit_src.php?bxpublic=Y&from=includefile&';
						$resize = 'true';
					break;
				}
			}
...
class CSiteTemplate
{
	function GetList($arOrder = Array(), $arFilter = Array())
	{
		...
		while(false !== ($file = @readdir($handle)))
		{
			...
			{
				__IncludeLang($path."/".$file."/lang/".LANGUAGE_ID."/description.php");
			}
			elseif(file_exists($path."/".$file."/lang/en/description.php"))
			{
				__IncludeLang($path."/".$file."/lang/en/description.php");
			}

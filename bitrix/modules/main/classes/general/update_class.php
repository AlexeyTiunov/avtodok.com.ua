...
class CUpdateSystem
{
	...
	function CheckVersions(&$arRes, &$strError, $arSelectedModules = false)
	{
		...
		if (strlen($strError_tmp)<=0)
		{
			for ($i = 0; $i < count($arModulesList); $i++)
			{
				if ($arSelectedModules===false || in_array($arModulesList[$i]["@"]["ID"], $arSelectedModules))
				{
					...
					{
						for ($j = 0; $j < count($arModulesList[$i]["#"]["VERSION_CONTROL"]); $j++)
						{
							...
							if ($bNeedInstall)
							{
								for ($k = 0; $k < count($arModulesList); $k++)
								{
...
class CUpdater
{
	...
	var $moduleID;	// Модуль
	var $callType; // Прямой вызов (ALL - все, KERNEL - ядро, DB - база данных)
	...
	{
		if ($this->callType == "KERNEL")
			return False;

		CUpdateSystem::AddMessage2Log("Run updater '".$this->updater."': Query(".$query.", ".$tableName.")", "CRUPDCDF2");
	...
	{
		return ($this->callType != "KERNEL");
	}

	function CanUpdateKernel()
	{
		return ($this->callType != "DB");
	}
}
?>
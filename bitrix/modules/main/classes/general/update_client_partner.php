	...
	function __CollectRequestData(&$strError, $lang = false, $stableVersionsOnly = "Y", $arRequestedModules = array(), $arAdditionalData = array(), $bStrongList = false)
	{
		...
		if (strlen($strError_tmp) <= 0)
		{
			...
			$strResult = "utf=".urlencode(defined('BX_UTF') ? "Y" : "N").
				...
				"&SUPD_STS=".urlencode(CUpdateClientPartner::__GetFooPath("GetList")).
				"&SUPD_URS=".urlencode(CUpdateClientPartner::__GetFooPath1("GetList")).
...
//	}
	...
	function __GetFooPath1($path)
	{
		$dbRes = $GLOBALS["DB"]->Query("SELECT COUNT(ID) as C FROM b_user WHERE ACTIVE = 'Y' AND LAST_LOGIN IS NOT NULL");

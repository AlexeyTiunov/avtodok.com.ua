...
/*  ÓÌÂˆ Ì‡ÒÚÓÈÍË Ô‡‡ÏÂÚÓ‚ ‚˚ÁÓ‚‡ ÒÚ‡ÌËˆ˚ */
...
if ($TYPE=="UPDATE" && $STEP==1)
{
	...
	<?
	// ≈ÒÎË ÂÒÚ¸ Ó·ÌÓ‚ÎÂÌËÂ ÒËÒÚÂÏ˚ Ó·ÌÓ‚ÎÂÌËÈ
	if ($arTypesUpdates!==false
		&& isset($arTypesUpdates["UPDATE_SYSTEM"]))
	{
		?>
	...
	<?
	$newLicense = COption::GetOptionString("main", "new_license7_sign", "N");
	if (!$bLockUpdateSystemKernel && $newLicense == "Y")
	{
		// ≈ÒÎË ÏÓÊÌÓ Â„ËÒÚËÓ‚‡Ú¸ ÔÓ‰ÛÍÚ
		if ($arTypesUpdates!==false
			&& defined("DEMO") && DEMO=="Y"
			&& isset($arTypesUpdates["CLIENT"])
			&& !isset($arTypesUpdates["UPDATE_SYSTEM"])
		...
		<?
		// ≈ÒÎË ÏÓÊÌÓ Á‡Í‡˜‡Ú¸ ËÒıÓ‰Ì˚Â ÚÂÍÒÚ˚
		if ($arTypesUpdates!==false

...
class CAllUserTypeManager
{
	...
	function GetUserType($user_type_id = false)
	{
		if(!is_array($this->arUserTypes))
		{
			...
			$db_events = GetModuleEvents("main", "OnUserTypeBuildList");
	...
	*/
	function GetUserFields($entity_id, $value_id=0, $LANG=false)
	{
		...
		if(!array_key_exists($entity_id.".".$LANG, $this->arFieldsCache))
		{
			...
			while($ar = $rs->Fetch())
			{
				$ar["USER_TYPE"] = $this->GetUserType($ar["USER_TYPE_ID"]);
				$ar["VALUE"] = false;
				$ar["ENTITY_VALUE_ID"] = $value_id;
				$result[$ar["FIELD_NAME"]] = $ar;
			}
		...
		foreach($arFilter as $key=>$value)
		{
			...
			else
			{
				...
				if(array_key_exists($key, $this->fields))
				{
					...
					if(
						(
							($operation=="I" || $operation=="E" || $operation=="S")
							&& (
								!is_array($value)
								&& (
						...
						|| (
							($operation=="NI" || $operation=="N" || $operation=="NS" || $operation=="NB")
							&& (
								is_array($value)
								|| (
									($FIELD_TYPE=="int" && intval($value)!=0)
							...
							switch($operation)
							{
								case "I":
									...
									elseif(strlen($FIELD_VALUE)<=0)
										$result[] = "(".$FIELD_NAME." IS NULL OR ".$length_func."(".$FIELD_NAME.") = 0)";
									else
										$result[] = $FIELD_NAME." = '".$FIELD_VALUE."'";
									break;
								case "E":
								...
								case "NI":
									if(is_array($FIELD_VALUE))
										$result[] = $FIELD_NAME." not in ('".implode("', '", $FIELD_VALUE)."')";
									elseif(strlen($FIELD_VALUE)<=0)
										$result[] = "(".$FIELD_NAME." IS NOT NULL AND ".$length_func."(".$FIELD_NAME.") > 0)";
									else
										$result[] = $FIELD_NAME." <> '".$FIELD_VALUE."'";

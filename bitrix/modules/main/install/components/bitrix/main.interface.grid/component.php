...
foreach($arParams["FILTER"] as $field)
{
	...
	if(is_array($flt) && !empty($flt) || !is_array($flt) && $flt <> '')
		...
		$arResult["FILTER_ROWS"][$field["id"]] = array_key_exists($field["id"], $aVisRows);
}
if(!isset($aOptions["filter_rows"]))
{
	foreach($arParams["FILTER"] as $field)
	{
		$arResult["FILTER_ROWS"][$field["id"]] = true;
...
$path = $APPLICATION->GetCurPageParam("", array("bxajaxid", "AJAX_CALL", $arParams["SORT_VARS"]["by"], $arParams["SORT_VARS"]["order"]));

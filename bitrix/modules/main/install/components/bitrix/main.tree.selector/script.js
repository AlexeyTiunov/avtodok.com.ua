function JCTreeSelectControl(arParams)
{
	...
	this.Run = function()
	{
		...
		if (_this._control.value && _this._control.value != _this._control.value_tmp)
		{
			...
			jsAjaxUtil.LoadData(url, _this.SetResult);
		}
	}
	...
	this.SearchShow = function()
	{
		if (null == _this._div)
		{
			var pos = jsUtils.GetRealPos(_this._control);

			//this._div = document.body.appendChild(document.createElement('DIV'));
			_this._div = document.getElementById('_f_popup_content').insertBefore(document.createElement('DIV'), document.getElementById('_f_popup_content').firstChild);
			_this._div.className = 'mts-search-result';

			//this._div.style.top = (pos.bottom + 2) + 'px';
			//this._div.style.left = (pos.left + 3) + 'px';
			if(jsUtils.IsIE())
				_this._div.style.top = pos.bottom-pos.top + 'px';
			else
				_this._div.style.top = (pos.bottom-pos.top+22) + 'px';
	...
	this.__control_keypress = function(e)
	{
		...
		switch (e.keyCode)
		{
			...
			case 38: //up
				...
				{
					_this._result[this._search_focus]._row.className = 'mts-search-result-row';
					_this._search_focus--;
	...
	this.LoadSection = function(SECTION_ID, bShowOnly, bScrollToSection)
	{
		...
		if (null == obSection.BX_LOADED)
		{
			...
			{
				jsAjaxUtil.LoadData(url, this.ShowSection);
			}
		}

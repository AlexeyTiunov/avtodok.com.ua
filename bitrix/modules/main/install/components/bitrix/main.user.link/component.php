...
if (!function_exists('MULChangeOnlineStatus') && $bSocialNetwork && !array_key_exists("IS_ONLINE", $arParams)) 
{
	...
	{
		static $arUserList, $arUserListID;
		...
		else
		{
			...
			while($arUser = $rsUser->Fetch()):
				...
				$arUserListOnline[$arUser["ID"]] = $online;
			endwhile;

			?><script type="text/javascript">
				jsUtils.addEvent(window, "load", function() { 
				<?
				foreach ($arUserList as $arTmpUser)
				{
					if ($arUserListOnline[$arTmpUser["USER_ID"]])
					{
						$classname = "bx-user-info-online";
						$title = GetMessage("MAIN_UL_ONLINE");
					}
					else
					{
						$classname = "bx-user-info-offline";
						$title = "";					
					}
						
					?>
					elOnline = document.getElementById('<?=$arTmpUser["HTML_ID"]?>');
					if(elOnline)
					{
						elOnline.className = "<?=$classname;?>";
						elOnline.title = "<?=$title;?>";
					}
					<?				
				}
				?>
...
if (strlen($arResult["FatalError"]) <= 0)
{
	...
	if ($bNeedGetUser)
	{
		...
		if (strlen($arResult["FatalError"]) <= 0 && $arParams['AJAX_CALL'] == 'INFO')
		{
			...
			{
				$strOnclick = "window.open('".$arResult["Urls"]["SonetMessageChat"]."', '', 'location=yes,status=no,scrollbars=yes,resizable=yes,width=700,height=550,top='+Math.floor((screen.height - 550)/2-14)+',left='+Math.floor((screen.width - 700)/2-5)); return false;";

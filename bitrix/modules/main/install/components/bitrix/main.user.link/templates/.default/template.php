...
else
{
	...
	if ($arParams["INLINE"] != "Y"):
		...
		<? if ($arResult["bSocialNetwork"]):
			...
			{
				if ($arParams["IS_ONLINE"] === true)
					$online_class_attrib = ' class="bx-user-info-online"';
				else
					$online_class_attrib = ' class="bx-user-info-offline"';
			}
			else
				$online_class_attrib = '';
		...
		echo (strlen($arResult["User"]["NAME_DESCRIPTION"]) > 0 ? " (".$arResult["User"]["NAME_DESCRIPTION"].")": "");

...
class BXInstallServices
{
	function CreateWizardIndex($wizardName, &$errorMessage)
	{
		...
		if (defined("WIZARD_DEFAULT_SITE_ID"))
		{
			...
			$p = CSite::GetSiteDocRoot($LID).$arSite["DIR"]."/index.php";
		}
	...
	function LoadWizardData($wizard)
	{
		...
		if (!array_key_exists("WIZARD_TYPE", $arWizardDescription))
			return false;
		if (defined("WIZARD_DEFAULT_TONLY") && WIZARD_DEFAULT_TONLY === true && strtoupper($arWizardDescription["WIZARD_TYPE"]) != "INSTALL"
			|| (!defined("WIZARD_DEFAULT_TONLY") || WIZARD_DEFAULT_TONLY !== true) && strtoupper($arWizardDescription["WIZARD_TYPE"]) != "INSTALL" && strtoupper($arWizardDescription["WIZARD_TYPE"]) != "INSTALL_ONCE")
			return false;
		...
		return array(
			"ID" => implode(":", $ar),
			"NAME" => (BXInstallServices::IsUTFString($arWizardDescription["NAME"]) ? mb_convert_encoding($arWizardDescription["NAME"], PRE_INSTALL_CHARSET, "utf-8") : $arWizardDescription["NAME"]),
			"DESCRIPTION" => (BXInstallServices::IsUTFString($arWizardDescription["DESCRIPTION"]) ? mb_convert_encoding($arWizardDescription["DESCRIPTION"], PRE_INSTALL_CHARSET, "utf-8") : $arWizardDescription["DESCRIPTION"]),
	...
	function CopyDirFiles($path_from, $path_to, $rewrite = true, $dirPermission = 0755, $filePermission = 0644)
	{
		...
		if (is_dir($path_from))
		{
			BXInstallServices::CheckDirPath($path_to."/", $dirPermission);
		}
		elseif (is_file($path_from))
		{
			$p = strrpos($path_to, "/");
			$path_to_dir = substr($path_to, 0, $p);
			BXInstallServices::CheckDirPath($path_to_dir."/", $dirPermission);
		...
		if ($handle = @opendir($path_from))
		{
			while (($file = readdir($handle)) !== false)
			{
				...
				{
					BXInstallServices::CopyDirFiles($path_from."/".$file, $path_to."/".$file, $rewrite, $dirPermission, $filePermission);
				}
				elseif (is_file($path_from."/".$file))
				{
					...
					@chmod($path_to."/".$file, $filePermission);
				}
			}
			@closedir($handle);
		}

	...
	function OnPostForm()
	{
		...
		if ($wizard->IsNextButtonClick())
		{
			...
			if ($createSite == "Y")
			{
				...
				$wizard->SetVar("siteCreate", "Y"); //COption::SetOptionString("main", "site_corp_services_create", "Y");
				//	COption::SetOptionString("main", "site_corp_services_id", $siteNewID);
				$wizard->SetVar("siteFolder", $siteFolder); //	COption::SetOptionString("main", "site_corp_services_folder", $siteFolder);
			}
			elseif (strlen($siteID) > 0)
			{
				...
				//COption::SetOptionString("main", "site_corp_services_create", "N");
				//COption::SetOptionString("main", "site_corp_services_id", $siteID);
				//COption::RemoveOption("main", "site_corp_services_folder");
				return;
			}
			else
			{
		...
		if($wizard->GetVar("siteCreate")=="Y")
		{
			...
			if (!($db_res && $res = $db_res->Fetch()))
			{
				$arFields = array(
					...
					"SORT"				=> 100,
					"DEF"				=> "N",
					"NAME"				=> (GetMessage("wiz_site_name")!=""?GetMessage("wiz_site_name"):GetMessage("wiz_site_default_name")),
					"DIR"				=> $wizard->GetVar("siteFolder"),
					"FORMAT_DATE"		=> "DD.MM.YYYY",
					"FORMAT_DATETIME"	=> "DD.MM.YYYY HH:MI:SS",
					"CHARSET"			=> (defined("BX_UTF") ? "UTF-8" : "windows-1251"),
					"SITE_NAME"			=> (GetMessage("wiz_site_name")!=""?GetMessage("wiz_site_name"):GetMessage("wiz_site_default_name")),

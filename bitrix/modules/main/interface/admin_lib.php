...
class CAdminPage
{
	...
	function ShowCSS()
	{
		...
		$arCSS = array_merge(array(
				...
				ADMIN_THEMES_PATH.'/'.ADMIN_THEME_ID.'/adminstyles.css'.'?'.filemtime($_SERVER["DOCUMENT_ROOT"].ADMIN_THEMES_PATH.'/'.ADMIN_THEME_ID.'/adminstyles.css'),
			),
			$this->GetModulesCSS(),
			$GLOBALS["APPLICATION"]->GetCSSArray()
		);
	...
	function GetModulesCSS($module_id='')
	{
		if($module_id <> '' && $this->aModules[$module_id] <> '')
		{
			...
			if(file_exists($fname))
				return array(ADMIN_THEMES_PATH.'/'.ADMIN_THEME_ID.'/'.$module_id.'.css'.'?'.filemtime($fname));
		}
		...
		if(file_exists($css_file))
			return array(ADMIN_THEMES_PATH.'/'.ADMIN_THEME_ID.'/modules.css'.'?'.filemtime($css_file));
...
<script type=\"text/javascript\" src=\"/bitrix/js/main/utils.js".'?'.filemtime($_SERVER["DOCUMENT_ROOT"].'/bitrix/js/main/utils.js')."\"></script>
<script type=\"text/javascript\" src=\"/bitrix/js/main/admin_tools.js".'?'.filemtime($_SERVER["DOCUMENT_ROOT"].'/bitrix/js/main/admin_tools.js')."\"></script>
<script type=\"text/javascript\" src=\"/bitrix/js/main/popup_menu.js".'?'.filemtime($_SERVER["DOCUMENT_ROOT"].'/bitrix/js/main/popup_menu.js')."\"></script>
";
	...
	function ShowSound()
	{
		...
		if($USER->IsAuthorized() && !isset($_COOKIE[COption::GetOptionString("main", "cookie_name", "BITRIX_SM").'_SOUND_LOGIN_PLAYED']))
		{
			...
			if($aUserOptGlobal["sound"] == 'Y')
			{
				...
				$res = '
<script type="text/javascript" src="/bitrix/components/bitrix/player/mediaplayer/flvscript.js'.'?v='.filemtime($_SERVER['DOCUMENT_ROOT'].'/bitrix/components/bitrix/player/mediaplayer/flvscript.js').'"></script>
...
class CAdminCalendar
{
	function ShowScript()
	{
		...
		return
"<script type=\"text/javascript\" src=\"/bitrix/js/main/calendar.js".'?'.filemtime($_SERVER["DOCUMENT_ROOT"].'/bitrix/js/main/calendar.js')."\"></script>".
"<script>
var mess = {
	...
	function EndContent()
	{
		if ($this->bContentStarted)
		{
			...
			if ($this->bContentBuffered)
			{
				...
				//ob_end_clean();
?></div><script type="text/javascript"><?=$this->jsPopup?>.SwapContent(BX('<?echo $this->cont_id?>'))</script><?
			}
...
function ShowJSHint($text, $arParams=false)
{
	...
	$res = '<script>window.hintonload_'.$tmp_id.' = function(img){if(img){jsUtils.appendBXHint(img, \''.$text.'\');}};</script>';
	$res .= '<img src="/bitrix/images/1.gif" onload="if (window.hintonload_'.$tmp_id.'){hintonload_'.$tmp_id.'(this);}">';

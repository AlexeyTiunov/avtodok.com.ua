...
if($bShowExtTime)
{
	...
	echo '<a href="javascript:jsDebugTimeWindow.Show(); jsDebugTimeWindow.ShowDetails(\'BX_DEBUG_TIME_1_1\')">'.GetMessage("debug_info_cr_time").'</a> '.round($PAGE, 4).' '.GetMessage("debug_info_sec").'<br>';
}
elseif($bShowTime)
{
	echo GetMessage("debug_info_cr_time").' '.round($main_exec_time, 4).' '.GetMessage("debug_info_sec").'<br />';
}
			...
			<div id="BX_DEBUG_TIME_1_<?echo $arArea["NUM"]?>" style="display:none">
				...
				?>
					<tr>
						...
						?></td>
						<td class="number" nowrap><?echo number_format((1-$tim/$arArea["TIME"])*100, 2)?>%</td>
				...
				<?$j=1;$k=1;foreach($APPLICATION->arIncludeDebug as $i=>$arIncludeDebug):?>
					...
					<tr>
						...
						?></td>
						<td class="number" nowrap><?echo number_format($arIncludeDebug["TIME"]/$arArea["TIME"]*100, 2)?>%</td>

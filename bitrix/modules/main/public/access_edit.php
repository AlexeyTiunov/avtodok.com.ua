...
do
{
	...
	<tr>
		...
		<td>
			...
			<?if ($arGroup["ID"] == "*" && !in_array(1, $arCurrentUserGroup)): //If not admin disable all group edit?>
...
window.BXCreateAccessHint = function()
{
	...
	oBXHint = new BXHint("<?=GetMessage("EDIT_ACCESS_PERMISSION_INFO")?>");
	...
	for (var index = 0; index < jsInheritPermIDs.length; index++)
		oBXHint = new BXHint("<?=GetMessage("EDIT_ACCESS_SET_PERMISSION")?>", document.getElementById("bx_permission_view_"+ jsInheritPermIDs[index]), {"width":200});
}

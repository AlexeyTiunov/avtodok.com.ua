...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_REQUEST["save"]) && $strWarning == "")
{
	...
	{
		$arParams = array(
			"path"=>($createNewFolder? $path.$fileName."/" : $path), 
...
<script>
	...
	(new BX.CAdminDialog({
		content_url: "/bitrix/admin/public_file_edit.php?<?=$_GET['subdialog'] == 'Y' ? "subdialog=Y" : ""?>&bxpublic=Y&lang=<?=CUtil::JSEscape($lang)?>"+(jsUtils.IsEditor() ? "" : "&noeditor=Y")+"&path=<?=urlencode($pathToEdit)?>&site=<?=$site?>&templateID=<?=urlencode($_REQUEST['templateID'])?>&back_url=<?=urlencode($relativePath)?>",
		height: 570, width: 780,
		resizable: false
	})).Show();
...
<?
$wiz_template_html = '';
if($obPageTemplate !== false && ($wiz_template_html = $obPageTemplate->GetFormHtml()) <> ''):
?>
...
window.bxNewPageWizard.AddStep("bx_new_page_common", {
	"next":"<?=(!empty($arMenu) ? "bx_new_page_menu" : ($wiz_template_html<>''? "bx_new_page_template":"bx_new_page_prop"))?>", 
...
window.bxNewPageWizard.AddStep("bx_new_page_menu", {
	...
	"next":"<?=($wiz_template_html <> ''? "bx_new_page_template":"bx_new_page_prop")?>", 
...
window.bxNewPageWizard.AddStep("bx_new_page_prop", {
	"prev": "<?=($wiz_template_html <> ''? "bx_new_page_template" : (!empty($arMenu) ? "bx_new_page_menu" : "bx_new_page_common"))?>", 
	"finish":"bx_new_page_prop", 
	"onfinish" : BXNewPageSave 
<?if($wiz_template_html == ''):?>,
	"onprev" : BXThirdStepPrev
<?endif?>
});

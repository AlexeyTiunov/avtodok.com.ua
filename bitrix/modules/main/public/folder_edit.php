...
if (isset($_REQUEST["path"]) && strlen($_REQUEST["path"]) > 0)
{
	...
	//Find real folder
	while($path != "/" && !is_dir($documentRoot.$path))
	{
	...
	//Cut / from the end
	if ( ($pathLength = strlen($path)) > 1 && $path[$pathLength-1] == "/")
		$path = substr($path, 0, -1);

}

if (strlen($path) <= 0 || !is_dir($documentRoot.$path))
	$path = "/";
...
elseif ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_REQUEST["save"]))
{
	...
	if($bNeedSectionFile)
		...
		@unlink($documentRoot.$path."/.section.php");
...
{
	$sSectionName = "";
	if(file_exists($absolutePath."/.section.php"))
		include($absolutePath."/.section.php");
}

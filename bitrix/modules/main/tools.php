...
function SelectBox(
	$strBoxName,					// ??? ????????
	$a,								// ??????? ? ?????? REFERENCE, REFERENCE_ID
	$strDetText = "",				// ?????? ??????? ?????? ? value = NOT_REF
	$strSelectedVal = "",			// ????????? ???????
...
function FormatDate($format="", $timestamp="")
{
	...
	elseif(preg_match_all("/[FfMlD]/", $format, $matches))
	{
		$ar = preg_split("/[FfMlD]/", $format);
		$result = "";
		foreach($matches[0] as $i=>$match)
		{
			switch($match)
			{
				case "F":$match=GetMessage("MONTH_".date("n", $timestamp)."_S");break;
				case "f":$match=GetMessage("MONTH_".date("n", $timestamp));break;
				case "M":$match=GetMessage("MON_".date("n", $timestamp));break;
				case "l":$match=GetMessage("DAY_OF_WEEK_".date("w", $timestamp));break;
				case "D":$match=GetMessage("DOW_".date("w", $timestamp));break;
			}
...
function HTMLToTxt($str, $strSiteUrl="", $aDelete=array(), $maxlen=70)
{
	...
	//???? ??????
	$str = preg_replace("/(<a\s.*?href\s*=\s*)([\"']?)(\\/.*?)(\\2)(\s.+?>|\s*>)(.*?)<\\/a>/is", "\\6 [".chr(1).$strSiteUrl."\\3".chr(1)."] ", $str);
	$str = preg_replace("/(<a\s.*?href\s*=\s*)([\"']?)(.*?)(\\2)(\s.+?>|\s*>)(.*?)<\\/a>/is", "\\6 [".chr(1)."\\3".chr(1)."] ", $str);
...
function GetPagePath($page=false, $get_index_page=null, $obj=false)
{
	...
	//Decoding UTF uri
	$bUTF = CUtil::DetectUTF8($sPath);
	$sPath = urldecode($sPath);
	if($bUTF && !defined("BX_UTF"))
	{
		if($obj)
...
function __IncludeLang($path, $bReturnArray=false)
{
	...
	if($bFirstCall)
	{
		...
		{
			$arMess = __IncludeLang($fname, true);
...
function bitrix_sessid()
{
	...
	return md5($_SESSION["fixed_session_id"]);
}
...
class CUtil
{
		...
		if(is_array($arData))
		{
			...
			foreach($arData as $key => $value)
			{
				if($first)
					...
					$res .= ",".($bWS ? "\n" : '');
				$res .= "'".preg_replace("/[\r\n]+/", "", CUtil::addslashes($key))."':".CUtil::PhpToJSObject($value);
			}
			$res .= ($bWS ? "\n" : '')."}";
		...
		else
		{
			...
			if (substr($data, 0, 1) == '{') // object
			{
				...
				foreach ($arTokens as $token)
				{
					...
					$arResult[$arTokenData[0]] = CUtil::JsObjectToPhp($arTokenData[1]);
				}
			}
			elseif (substr($data, 0, 1) == '[') // array
			{
				...
				if (count($arCommaPos) > 0)
				{
					...
					foreach ($arCommaPos as $pos)
					{
						$arResult[] = CUtil::JsObjectToPhp(substr($token, $prev_index, $pos - $prev_index - 1));
						$prev_index = $pos;
					}
					$arResult[] = CUtil::JsObjectToPhp(substr($token, $prev_index));
				}
				else
				{
					$arResult[] = CUtil::JsObjectToPhp($token);
				}
			}
			else // scalar
			{
	...
	function InitJSCore($arExt = array(), $bReturn = false)
	{
		...
		static $arCurrentlyLoadedExt = array();
		...
		if ($bReturn)
		{
			if (!$arCurrentlyLoadedExt['core'])
				$str .= '<script type="text/javascript" src="/bitrix/js/main/core/core.js"></script>'."\r\n";
		}
		else
		{
			$APPLICATION->AddHeadString('<script type="text/javascript" src="/bitrix/js/main/core/core.js"></script>', true);
		}

		$mess_lang = __IncludeLang($_SERVER['DOCUMENT_ROOT'].BX_ROOT.'/modules/main/lang/'.LANGUAGE_ID.'/js_core.php', true);
		$mess = array(
		...
		if ($bReturn)
		{
			if (!$arCurrentlyLoadedExt['core'])
			{
				$str .= '<script type="text/javascript">BX.message('.CUtil::PhpToJSObject(array_merge($mess, $mess_lang), false).')</script>'."\r\n";
				$str .= '<link href="/bitrix/js/main/core/css/core.css" type="text/css" rel="stylesheet" />'."\r\n";
			}
		}
		else
		{
			$APPLICATION->AddHeadString('<script type="text/javascript">BX.message('.CUtil::PhpToJSObject(array_merge($mess, $mess_lang), false).')</script>', true);
			$APPLICATION->SetAdditionalCSS('/bitrix/js/main/core/css/core.css');
		}

		$arCurrentlyLoadedExt['core'] = true;

		for ($i = 0, $len = count($arExt); $i < $len; $i++)
		{
			$mess_lang_ext = array();
			$ext = preg_replace('/[^a-z0-9_]/i', '', $arExt[$i]);
			$fname = $_SERVER['DOCUMENT_ROOT'].BX_ROOT.'/modules/main/lang/'.LANGUAGE_ID.'/js_core_'.$ext.'.php';
			if (file_exists($fname))
			{
				$mess_lang_ext = __IncludeLang($fname, true);
			}


			if ($bReturn)
			{
				if (!$arCurrentlyLoadedExt[$arExt[$i]])
				{
					if (is_array($mess_lang_ext) && count($mess_lang_ext) > 0)
						$str .= '<script type="text/javascript">BX.message('.CUtil::PhpToJSObject($mess_lang_ext, false).')</script>'."\r\n";

					$str .= '<script type="text/javascript" src="/bitrix/js/main/core/core_'.$arExt[$i].'.js"></script>'."\r\n";

					if (file_exists($_SERVER['DOCUMENT_ROOT'].'/bitrix/js/main/core/css/core_'.$arExt[$i].'.css'))
						$str .= '<link href="/bitrix/js/main/core/css/core_'.$arExt[$i].'.css" type="text/css" rel="stylesheet" />'."\r\n";
				}
			}
			else
			{
				if (is_array($mess_lang_ext) && count($mess_lang_ext) > 0)
					$APPLICATION->AddHeadString('<script type="text/javascript">BX.message('.CUtil::PhpToJSObject($mess_lang_ext, false).')</script>', true);

				$APPLICATION->AddHeadString('<script type="text/javascript" src="/bitrix/js/main/core/core_'.$arExt[$i].'.js"></script>', true);

				if (file_exists($_SERVER['DOCUMENT_ROOT'].'/bitrix/js/main/core/css/core_'.$arExt[$i].'.css'))
					$APPLICATION->SetAdditionalCSS('/bitrix/js/main/core/css/core_'.$arExt[$i].'.css');
			}

			$arCurrentlyLoadedExt[$arExt[$i]] = true;
		}

		if (defined('BX_PUBLIC_MODE') && BX_PUBLIC_MODE == 1 && !$bOldReturn)
			echo $str;

		return $bReturn ? $str : true;
	}
...
function CheckSerializedData($str, $max_depth = 20)
{
	...
	preg_match_all("/\\{[^}]*?\\{/", $str, $matches); // check data depth
	if (is_array($matches) && count($matches[0])*2 > $max_depth)
		return false;
...
class UpdateTools
{
	function CheckUpdates()
	{
		...
		if($days_check > 0)
		{
			CUserOptions::SetOption('update', 'options', array('tooltip'=>'on'));
	...
	function GetUpdateResult()
	{
		...
		{
			$updOptions = CUserOptions::GetOption('update', 'options', array());
			if($updOptions['tooltip'] <> 'off')
			{

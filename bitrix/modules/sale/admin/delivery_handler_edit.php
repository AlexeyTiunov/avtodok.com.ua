...
foreach ($siteList as $arSite)
{
	...
	foreach ($arDeliveryInfo[$curSITE_ID]['PROFILES'] as $key => $arProfile)
	{
		...
		foreach ($arProfile["RESTRICTIONS_WEIGHT"] as $pkey => $value) 
			$arProfile["RESTRICTIONS_WEIGHT"][$pkey] = number_format($value, 2, '.', '');
		foreach ($arProfile["RESTRICTIONS_SUM"] as $pkey => $value) 
			$arProfile["RESTRICTIONS_SUM"][$pkey] = number_format($value, 2, '.', '');
...
if (is_array($arDeliveryInfo[$SITE_ID]["CONFIG"]))
{
	if (is_array($arDeliveryInfo[$SITE_ID]["CONFIG"]["CONFIG_GROUPS"]))
	{
		...
		{
			$configTabsCount++;
			$aTabs[] = array("DIV" => "edit_".htmlspecialchars($group), "TAB" => htmlspecialchars($title), "TITLE" => htmlspecialchars($title));
		}
	}
	else
	{
...
<script language="javascript">
var cur_site = '<?=htmlspecialchars($siteList[0]["ID"])?>';
		...
		else if (i == 'PROFILES')
		{
			for (var j in arStructure[cur_site]['PROFILES'])
			{
				for (var k in arStructure[cur_site]['PROFILES'][j])
				{
					...
					{
						if (k == 'RESTRICTIONS_SUM' && arStructure[cur_site]['PROFILES'][j][k].length <= 2)
							arStructure[cur_site]['PROFILES'][j][k] = {0:0,1:0,2:0};
						else if (arStructure[cur_site]['PROFILES'][j][k].length <= 1)
...
<form method="POST" action="<?echo $APPLICATION->GetCurPage()?>" name="form1" onSubmit='return prepareData()'>
	...
	<input type="hidden" name="SID" value="<?echo htmlspecialchars($SID) ?>" />
		...
		<td><select name="site" id="site_id"<? if(!$bSites) echo " disabled=\"disabled\""; ?> onChange="selectSite(this.value)">
			<?
				for($i = 0; $i < $siteCount; $i++)
					echo "<option value=\"".htmlspecialchars($siteList[$i]["ID"])."\" ".($i == 0 ? "selected=\"selected\"" : "").">".htmlspecialchars($siteList[$i]["NAME"])."</option>";
			?></select><input type="hidden" name="current_site" id="current_site" value="<?=htmlspecialchars($siteList[0]["ID"]);?>" /></td>
	</tr>
	<tr>
		<td colspan="2">
<?
...
<tr>
	...
	<td width="60%"><b><?=htmlspecialchars($handlerPath)?></b></td>
</tr>
<tr>
...
<tr>
	...
	<td><input type="text" name="HANDLER[NAME]" value="<?=htmlspecialchars($arDeliveryInfo[$SITE_ID]["NAME"])?>" /></td>
</tr>
<tr>
	<td valign="top" class="field-name"><?=GetMessage('SALE_DH_HANDLER_DESCRIPTION')?></td>
	<td valign="top"><textarea name="HANDLER[DESCRIPTION]" cols="40" rows="5"><?=htmlspecialchars($arDeliveryInfo[$SITE_ID]["DESCRIPTION"])?></textarea></td>
</tr>
<tr>
...
foreach ($arDeliveryInfo[$SITE_ID]["CONFIG"]["CONFIG_GROUPS"] as $group => $arConfigGroup)
{
	$tabControl->BeginNextTab();
?>
<table border="0" cellspacing="0" cellpadding="0" width="100%" class="edit-table" id="params_<?=htmlspecialchars($group)?>_table">
<?
	foreach ($arDeliveryInfo[$SITE_ID]["CONFIG"]["CONFIG"] as $config_id => $arConfig)
	{
...
?>
<tr>
	<td class="field-name" <?if ($arConfig["TYPE"] == "MULTISELECT"):?>valign="top" <?endif;?>width="40%" align="right"><?=htmlspecialchars($arConfig["TITLE"])?>:</td>
...
<?
	switch ($arConfig["TYPE"])
	{
		case "CHECKBOX":
			?>
			<input type="checkbox" name="HANDLER[CONFIG][<?=htmlspecialchars($config_id)?>]" value="Y" <?=($arConfigValues[$config_id] == "Y" ? "checked=\"checked\"" : "")?> />
			<?
		break;
		...
		?>
				<input type="radio" id="hc_<?=htmlspecialchars($config_id)?>_<?=htmlspecialcharsEx($value)?>" name="HANDLER[CONFIG][<?=htmlspecialchars($config_id)?>]" value="<?=htmlspecialcharsEx($value)?>"<?=($value == $arConfigValues[$config_id] ? " checked=\"checked\"" : "")?> /><label for="hc_<?=htmlspecialchars($config_id)?>_<?=htmlspecialcharsEx($value)?>"><?=htmlspecialcharsEx($title)?></label><br />
		<?
			endforeach;
		...
		case "PASSWORD":
		?>
			<input type="password" name="HANDLER[CONFIG][<?=htmlspecialchars($config_id)?>]" value="<?=htmlspecialchars($arConfigValues[$config_id])?>" />
		<?
		break;
		...
		case "MULTISELECT":
		?>
			<select name="HANDLER[CONFIG][<?=htmlspecialchars($config_id)?>]"<?=$arConfig["TYPE"] == "MULTISELECT" ? "multiple=\"multiple\"" : ""?>>
		<?
		...
		default:
		?>
			<input type="text" name="HANDLER[CONFIG][<?=htmlspecialchars($config_id)?>]" value="<?=htmlspecialchars($arConfigValues[$config_id])?>" />
		<?
	}
...
if (is_array($arDeliveryInfo[$SITE_ID]["PROFILES"]))
{
	...
	{
?>
		<table class="internal" cellpadding="0" cellspacing="2" border="0" width="100%" align="center" id="profile_<?=htmlspecialchars($profile_id)?>">
			<tr class="heading">
				<td width="40%">&nbsp;</td>
				<td width="60%">&nbsp;</td>
			</tr>
			<tr>
				...
				<td valign="top"><input type="checkbox" name="HANDLER[PROFILES][<?=htmlspecialchars($profile_id)?>][ACTIVE]" value="Y" <?if ($arProfile['ACTIVE'] == 'Y'):?>checked="checked"<?endif?> /></td>
			</tr>
			<tr>
				<td align="right"><?=GetMessage('SALE_DH_PROFILE_TITLE')?></td>
				<td valign="top"><input type="text" name="HANDLER[PROFILES][<?=htmlspecialchars($profile_id)?>][TITLE]" value="<?=htmlspecialchars($arProfile["TITLE"])?>" size="25" /></td>
			</tr>
			<tr>
				<td align="right" valign="top"><?=GetMessage('SALE_DH_PROFILE_DESCRIPTION')?></td>
				<td valign="top"><textarea name="HANDLER[PROFILES][<?=htmlspecialchars($profile_id)?>][DESCRIPTION]" cols="40" rows="5"><?=htmlspecialchars($arProfile["DESCRIPTION"])?></textarea></td>
			</tr>	
			...
			<tr>
				...
				<td valign="top">
					...
					<input type="text" name="HANDLER[PROFILES][<?=htmlspecialchars($profile_id)?>][RESTRICTIONS_WEIGHT][0]" value="<?=number_format($arProfile['RESTRICTIONS_WEIGHT'][0] / $weight_koef, 2, '.', '')?>" size="8" /><?=htmlspecialchars($weight_unit)?>
					<?=GetMessage('SALE_DH_PROFILE_WEIGHT_RESTRICTIONS_TO')?>
					<input type="text" name="HANDLER[PROFILES][<?=htmlspecialchars($profile_id)?>][RESTRICTIONS_WEIGHT][1]" value="<?=number_format($arProfile['RESTRICTIONS_WEIGHT'][1] / $weight_koef, 2, '.', '')?>" size="8" /><?=htmlspecialchars($weight_unit)?>
				</td>
			</tr>
			...
			<tr>
				...
				<td valign="top">
					...
					<input type="text" name="HANDLER[PROFILES][<?=htmlspecialchars($profile_id)?>][RESTRICTIONS_SUM][1]" value="<?=number_format(floatval($arProfile['RESTRICTIONS_SUM'][1]), 2, '.', '')?>" size="8" />&nbsp;
					<?=GetMessage('SALE_DH_PROFILE_SUM_RESTRICTIONS_TO')?>
					<input type="text" name="HANDLER[PROFILES][<?=htmlspecialchars($profile_id)?>][RESTRICTIONS_SUM][2]" value="<?=number_format(floatval($arProfile['RESTRICTIONS_SUM'][2]), 2, '.', '')?>" size="8" />
					<?=GetMessage('SALE_DH_PROFILE_SUM_RESTRICTIONS_CURRENCY')?>
					<?=CCurrency::SelectBox('HANDLER[PROFILES]['.htmlspecialchars($profile_id).'][RESTRICTIONS_SUM][0]', $arDeliveryInfo[$SITE_ID]["BASE_CURRENCY"])?>
				</td>
			</tr>
		</table><br />
<?

...
foreach ($arOrderPropsCode as $key => $value)
{
	if ($value["IS_FILTERED"] == "Y" && $value["TYPE"] != "MULTISELECT")
	{
		...
		{
			if($value["TYPE"]=="TEXT" || $value["TYPE"]=="TEXTAREA")
				$arFilter["%PROPERTY_VAL_BY_CODE_".$key] = Trim(${"filter_prop_".$key});
			else
				$arFilter["PROPERTY_VAL_BY_CODE_".$key] = Trim(${"filter_prop_".$key});
		}
	}
	...
	if ($bNeedBasket)
	{
		...
		while ($arItem = $dbItemsList->GetNext())
		{
			...
			if (strlen($arItem["DETAIL_PAGE_URL"]) > 0)
				$fieldValue .= "<a href=\"http://".$serverName[$arOrder["LID"]].$arItem["~DETAIL_PAGE_URL"]."\">";
			...
			if(strlen($arItem["NAME"]) > 0)
			{
				$fieldName .= "<nobr>";
				if (strlen($arItem["DETAIL_PAGE_URL"]) > 0)
					$fieldName .= "<a href=\"".$arItem["~DETAIL_PAGE_URL"]."\">";
			...
			if(strlen($arItem["WEIGHT"]) > 0)
				$fieldWeight .= $arItem["WEIGHT"]."&nbsp;".htmlspecialchars(COption::GetOptionString('sale', 'weight_unit'));
			else
				$fieldWeight .= "<br />";
			...
			if(strlen($arItem["DISCOUNT_VALUE"]) > 0)
				$fieldDiscountValue .= $arItem["DISCOUNT_VALUE"];
			else
...
?>
		...
		<td>
			<select name="filter_lang">
				...
				while ($arSitesList = $dbSitesList->Fetch())
				{
					...
					?><option value="<?= htmlspecialchars($arSitesList["LID"])?>"<?if ($arSitesList["LID"] == $filter_lang) echo " selected";?>>[<?= htmlspecialcharsex($arSitesList["LID"]) ?>]&nbsp;<?= htmlspecialcharsex($arSitesList["NAME"]) ?></option><?
				}
				?>
			</select>
			/
		...
		<td>
			<select name="filter_person_type[]" multiple size="3">
				...
				while ($personType = $l->Fetch()):
					?><option value="<?echo htmlspecialchars($personType["ID"])?>"<?if (is_array($filter_person_type) && in_array($personType["ID"], $filter_person_type)) echo " selected"?>>[<?echo htmlspecialchars($personType["ID"]) ?>] <?echo htmlspecialchars($personType["NAME"])?> <?echo "(".htmlspecialchars($personType["LID"]).")";?></option><?
				endwhile;
				?>
			</select>
		</td>
		...
		<td>
			<select name="filter_pay_system[]" multiple size="3">
				...
				while ($paySystem = $l->Fetch()):
					?><option value="<?echo htmlspecialchars($paySystem["ID"])?>"<?if (is_array($filter_pay_system) && in_array($paySystem["ID"], $filter_pay_system)) echo " selected"?>>[<?echo htmlspecialchars($paySystem["ID"]) ?>] <?echo htmlspecialchars($paySystem["NAME"])?> <?echo "(".htmlspecialchars($paySystem["LID"]).")";?></option><?
				endwhile;
				?>
			</select>
		</td>
	...
	<tr>
		...
		<td>
			<select name="filter_delivery[]" multiple size="3">
				...
				{
					?><option value="<?echo $arDelivery["ID"]?>"<?if (is_array($filter_delivery) && in_array($delivery_id, $filter_delivery)) echo " selected"?>>[<?echo $arDelivery["ID"]?>] <?echo $arDelivery["NAME"]?></option><?
				}
				?>
			</select>
		</td>
	...
	<tr>
		...
		<td>
			<input type="text" name="filter_affiliate_id" value="<?= htmlspecialchars($filter_affiliate_id) ?>" size="10" maxlength="10">

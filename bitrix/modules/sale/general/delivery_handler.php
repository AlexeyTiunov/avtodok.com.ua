...
class CAllSaleDeliveryHandler
{
	...
	function __getHandlersData($arFullHandlersList, $SITE_ID = false)
	{
		...
		$query = "
SELECT HID AS SID, LID, ACTIVE, NAME, SORT, DESCRIPTION, HANDLER, SETTINGS, PROFILES, TAX_RATE
...
WHERE HID IN (".$strKeys.")";
		...
		//echo "<pre>"; print_r($arHandlersList); echo "</pre>";
	...
	{
		$rsHandlersList = GetModuleEvents("sale", "onSaleDeliveryHandlersBuildList");
		$arHandlersList = array();
		if ($rsHandlersList->SelectedRowsCount() > 0)
		{
			while ($arHandler = $rsHandlersList->Fetch())
			{
	...
	*/
		...
		foreach ($arSort as $SORT => $SORT_DIR)
		{
			...
			{
				$arSort[$SORT] = $SORT_DIR == "DESC" ? "DESC" : "ASC";
			}
			else
				unset($arSort[$SORT]);
		}
...
";
	...
	function __executeCalculateEvents($SID, $profile, $arOrder, $arReturn)
	{
		...
		{
			$rsEventHandlersList = GetModuleEvents("sale", $event);
			if ($rsEventHandlersList->SelectedRowsCount() > 0)
			{
				while ($arEventHandler = $rsEventHandlersList->Fetch())
				{

...
class CAllSaleOrder
{
	...
	function PayOrder($ID, $val, $bWithdraw = True, $bPay = True, $recurringID = 0, $arAdditionalFields = array())
	{
		...
		while ($arEvent = $db_events->Fetch())
			if (ExecuteModuleEvent($arEvent, $ID, $val, $bWithdraw, $bPay, $recurringID, $arAdditionalFields)===false)
				return false;
		...
		while ($arEvent = $events->Fetch())
			ExecuteModuleEvent($arEvent, $ID, $val);
	...
	function DeliverOrder($ID, $val, $recurringID = 0, $arAdditionalFields = array())
	{
		...
		while ($arEvent = $db_events->Fetch())
			if (ExecuteModuleEvent($arEvent, $ID, $val, $recurringID, $arAdditionalFields)===false)
				return false;
	...
	function CancelOrder($ID, $val, $description = "")
	{
		...
		while ($arEvent = $db_events->Fetch())
			if (ExecuteModuleEvent($arEvent, $ID, $val)===false)
				return false;
	...
	function StatusOrder($ID, $val)
	{
		...
		{
			$arFields["TEXT"] = ExecuteModuleEvent($arEvent, $ID, $arStatus["ID"]);
		}

...
class CAllSaleOrderTax
{
	...
	// Функция не обрабатывает налоги с фиксированной ставкой. Только с процентной!
	// возвращает сумму всех налогов для данной цены
	// во втором аргументе ($arTaxList[]["TAX_VAL"]) возвращает величину данного налога для данной цены
	function CountTaxes($Price, &$arTaxList, $DefCurrency)
	{
		//1. Раскручиваем стэк включенных в цену налогов для определения начальной цены
		$part_sum = 0.00;
		$tax_koeff = 1.00;
		$minus = 0.00;
		...
		$tax_koeff += $part_sum;
		$item_price = RoundEx($Price/($tax_koeff-$minus), SALE_VALUE_PRECISION);
		...
		for ($i = 0; $i < count($arTaxList); $i++)
		{
			...
			$val = $tax_koeff * DoubleVal($arTaxList[$i]["VALUE"]) / 100.00;
			$tax_val = RoundEx($val*$item_price, SALE_VALUE_PRECISION);
			$part_sum += $val;
			$total_tax += $tax_val;

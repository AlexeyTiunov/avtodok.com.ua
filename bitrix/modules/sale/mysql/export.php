...
class CSaleExport extends CAllSaleExport
{
	/*************** ADD, UPDATE, DELETE *****************/
	function Add($arFields)
	{
		...
		$arInsert = $DB->PrepareInsert("b_sale_export", $arFields);
		...
		{
			$strSql =
				"INSERT INTO b_sale_export(".$arInsert[0].") ".
				"VALUES(".$arInsert[1].")";
	...
	function GetList($arOrder = Array("ID" => "DESC"), $arFilter = Array(), $arGroupBy = false, $arNavStartParams = false, $arSelectFields = array())
	{
		...
		if (count($arSelectFields) <= 0)
			$arSelectFields = array("ID", "PERSON_TYPE_ID", "VARS");

		// FIELDS -->
		$arFields = array(
				"ID" => array("FIELD" => "E.ID", "TYPE" => "int"),
				"PERSON_TYPE_ID" => array("FIELD" => "E.PERSON_TYPE_ID", "TYPE" => "int"),
				"VARS" => array("FIELD" => "E.VARS", "TYPE" => "string")
			);
		// <-- FIELDS

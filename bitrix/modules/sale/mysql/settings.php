...
class CSaleGroupAccessToSite extends CAllSaleGroupAccessToSite
{
	...
	function Add($arFields)
	{
		...
		$strSql =
			"INSERT INTO b_sale_site2group(".$arInsert[0].") ".
			"VALUES(".$arInsert[1].")";
...
class CSaleGroupAccessToFlag extends CAllSaleGroupAccessToFlag
{
	function GetList($arOrder = array(), $arFilter = array(), $arGroupBy = false, $arNavStartParams = false, $arSelectFields = array())
	{
		...
		$arFields = array(
				"ID" => array("FIELD" => "S.ID", "TYPE" => "int"),
				"GROUP_ID" => array("FIELD" => "S.GROUP_ID", "TYPE" => "int"),
				"ORDER_FLAG" => array("FIELD" => "S.ORDER_FLAG", "TYPE" => "char")
			);
		// <-- FIELDS
	...
	function Add($arFields)
	{
		...
		if (!CSaleGroupAccessToFlag::CheckFields("ADD", $arFields))
			return false;

		$arInsert = $DB->PrepareInsert("b_sale_order_flags2group", $arFields);

		$strSql =
			"INSERT INTO b_sale_order_flags2group(".$arInsert[0].") ".
			"VALUES(".$arInsert[1].")";
		$DB->Query($strSql, false, "File: ".__FILE__."<br>Line: ".__LINE__);

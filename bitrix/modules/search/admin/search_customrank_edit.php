...
if($ID>0)
{
	...
	$aMenu[] = array(
		...
		"LINK"=>"javascript:if(confirm('".AddSlashes(GetMessage("customrank_edit_delete_conf"))."'))window.location='search_customrank_admin.php?ID=".$ID."&cf=delid&lang=".LANG."&".bitrix_sessid_get()."';",
		"ICON"=>"btn_delete",
	);
}
...
?>
	...
	<tr>
		...
		<?if(IsModuleInstalled('iblock') || $str_MODULE_ID=="iblock"):?>
			<option value="iblock"<?=$str_MODULE_ID=="iblock"?" selected":""?>><?=GetMessage("customrank_edit_iblocks")?></option>
		<?endif?>
		<?if(IsModuleInstalled('blog') || $str_MODULE_ID=="blog"):?>
			<option value="blog"<?=$str_MODULE_ID=="blog"?" selected":""?>><?=GetMessage("customrank_edit_blog")?></option>
		<?endif?>
		<?if(IsModuleInstalled('forum') || $str_MODULE_ID=="forum"):?>
			<option value="forum"<?=$str_MODULE_ID=="forum"?" selected":""?>><?=GetMessage("customrank_edit_forum")?></option>
		<?endif?>
		<?if(IsModuleInstalled('socialnetwork') || $str_MODULE_ID=="socialnetwork"):?>
			<option value="socialnetwork"<?=$str_MODULE_ID=="socialnetwork"?" selected":""?>><?=GetMessage("customrank_edit_socialnetwork")?></option>
		<?endif?>
		<?if(IsModuleInstalled('intranet') || $str_MODULE_ID=="intranet"):?>
			<option value="intranet"<?=$str_MODULE_ID=="intranet"?" selected":""?>><?=GetMessage("customrank_edit_intranet")?></option>
		<?endif?>
		</select>
		</td>
	</tr>

...
if($_SERVER["REQUEST_METHOD"] == "POST" && $_REQUEST["Reindex"]=="Y")
{
	...
	if($Full!="N" && $NS["CLEAR"]!="Y")
	{
		if(!check_bitrix_sessid())
			$NS["CLEAR"] = "Y";
	}

	$res = CSearch::ReIndexAll($Full!="N", COption::GetOptionInt("search", "max_execution_time"), $NS, $_REQUEST["clear_suggest"]==="Y");
	if(is_array($res)):
...
function DoNext(NS)
{
	...
	if(!NS)
	{
		...
		queryString += '&max_execution_time='+jsUtils.urlencode(document.getElementById('max_execution_time').value);
	}
...
?>
	...
	<tr>
		...
		<?if(IsModuleInstalled('intranet')):?>
			<option value="intranet"><?=GetMessage("SEARCH_REINDEX_INTRANET")?></option>
		<?endif;?>
		</select>
		</td>
	</tr>

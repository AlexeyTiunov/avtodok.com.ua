...
class CAllSearch extends CDBResult
{
	...
	function ReindexFile($path, $SEARCH_SESS_ID="")
	{
		...
		while($arEvent = $events->Fetch())
		{
			if($arrFile = ExecuteModuleEventEx($arEvent, array($DOC_ROOT."/".$path, $SEARCH_SESS_ID)))
				break;
		}
		if(!is_array($arrFile))
		{
			$sFile = $APPLICATION->GetFileContent($DOC_ROOT."/".$path);
	...
	function __PrepareFilter($arFilter, &$bIncSites, $strSearchContentAlias="sc.")
	{
		...
		foreach($arFilter as $field=>$val)
		{
			...
			if(is_array($val) && count($val) == 1 && $field !== "URL")
				$val = $val[0];
			switch($field)
			{
			case "MODULE_ID":

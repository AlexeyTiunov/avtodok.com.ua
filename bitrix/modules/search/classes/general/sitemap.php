	...
	function Create($site_id, $max_execution_time, $NS, $arOptions = array())
	{
		...
		if($arSite=$rsSite->Fetch())
		{
			...
			while(!$bFileIsFull && $ar=$this->Fetch())
			{
				...
				$strURL = $this->LocationEncode("http://".$ar["SERVER_NAME"].$ar["URL"]);
			...
			for($i = 0; $i <= $NS["FILE_ID"]; $i++)
			{
				...
				fwrite($f,"\t<sitemap>\n\t\t<loc>http://".$arSite["SERVER_NAME"].$strFile."</loc>\n\t\t<lastmod>".$strTime."</lastmod>\n\t</sitemap>\n");
			}
			fwrite($f,"</sitemapindex>\n");
			fclose($f);
			$this->m_errors_count=$NS["ERROR_CNT"];
			$this->m_errors_href="http://".$arSite["SERVER_NAME"].$arSite["DIR"]."sitemap_errors.xml";
			$this->m_href="http://".$arSite["SERVER_NAME"].$arSite["DIR"]."sitemap_index.xml";
			return true;
		}
		else
		{

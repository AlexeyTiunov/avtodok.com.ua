...
class CSecurityAntiVirus
{
			...
			if($arInfo = $rsInfo->Fetch())
			{
				if($table_lock = CSecurityDB::LockTable('b_sec_virus', $APPLICATION->GetServerUniqID()."_virus"))
				{
					...
					do {
						...
						$DB->Query("update b_sec_virus set SENT='Y' where ID='".$arInfo["ID"]."'");
					...
					$DB->Query("DELETE FROM b_sec_virus WHERE TIMESTAMP_X <= ".$DB->CharToDateFunction(ConvertTimeStamp($date, "FULL")));
	...
	function isinwhitelist()
	{
		...
		if(strpos($this->body, "var cmt") !== false)
			return 1;

		if(strpos($this->body, "MoveProgress(") !== false)
			return 1;

		if(strpos($this->body, "Import(") !== false)
			return 1;

		if(strpos($this->body, "bx_template_params") !== false)
			return 1;

		if(strpos($this->body, "JCMenu(") !== false)
			return 1;
		...
		if(preg_match("/(jsAjaxUtil|jsUtils|jsPopup|elOnline|jsAdminChain|jsEvent|jsAjaxHistory)\\./", $this->body))
			return 1;

		if(preg_match("/new\\s+(PopupMenu|JCAdminFilter|JCAdminMenu|BXHint)/", $this->body))
			return 1;

		if(strpos($this->body, "document\\.write('<link href=\"/bitrix/templates/") !== false)
			return 1;

		if(strpos($this->body, "document.getElementById('session_time_result').innerHTML") !== false)
			return 1;

		if(preg_match("/(structRegisterDD|bx_adv_includeFlash)/", $this->body))
			return 1;

		if(preg_match("/(iblock_element_edit|iblock_element_search|posting_admin|fileman_file_view|sale_print)\\.php/", $this->body))
			return 1;

		if(preg_match("/BX\\.(WindowManager|reload|message)/", $this->body))
			return 1;
		...
		if(preg_match("/var\\s+(updateURL|bx_incl_area)\\s+=/", $this->body))
			return 1;
		...
		if(preg_match("/window\\.(bx_req_res|MLSearchResult|arUsedCSS|arComp2Templates|arComp2TemplateProps|arComp1Elements|arSnippets|JCCalendarViewMonth|JCCalendarViewWeek|JCCalendarViewDay|_bx_result|_bx_new_event|_bx_plann_mr|_bx_ar_events|_bx_calendar|_bx_plann_events|_bx_existent_event|_ml_items_colls|MLCollections|fmsBtimeout|fmsResult|arSnGroups|BXFM_result|BXFM_NoCopyToDir)/", $this->body))
			return 1;

		if(
			$this->body === "window.location.reload();"
			|| $this->body === "window.location = window.location.href;"
			|| preg_match("/^parent\\.window\\.End\\(\\d+\\);$/", $this->body)
			|| preg_match("/^top\\.location\\.href\\s*=\\s*(['\"])[^'\"]*\\1;{0,1}$/", $this->body)
			|| preg_match("/\\.setTimeout\\('CheckNew\\(\\)'/", $this->body)
		)
			return 1;

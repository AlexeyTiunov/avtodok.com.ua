	...
	function OnPostForm()
	{
		...
		if ($wizard->IsNextButtonClick())
		{
			...
			if ($createSite == "Y")
			{
				...
				COption::SetOptionString("main", "site_personal_create", "Y");
				
				COption::SetOptionString("main", "site_personal_id", $siteNewID);
				COption::SetOptionString("main", "site_personal_folder", $siteFolder);
			}
			elseif (strlen($siteID) > 0)
			{
				...
				COption::SetOptionString("main", "site_personal_id", $siteID);
				COption::RemoveOption("main", "site_personal_folder");
				return;
			}
			else
			{
...
class SiteSettingsStep extends CWizardStep
{
	...
	function ShowStep()
	{
		...
		$this->content .= $this->ShowCheckboxField("installDemoData", "Y", 
			(array("id" => "install-demo-data") + ($_SERVER["PHP_SELF"] == "/index.php" ? array() : array("checked" => false))));
...
class DataInstallStep extends CWizardStep
{
	...
	function OnPostForm()
	{
		...
		$arServices = WizardServices::GetServices($_SERVER["DOCUMENT_ROOT"].$wizard->GetPath(), "/site/services/");
			...
			{
				$arServices = array("main" => $arServices["main"], "blog" => $arServices["blog"]); 
			}
	...
	function InstallService($serviceID, $serviceStage)
	{
		...
		//define("WIZARD_REINSTALL_DATA", false); // убрать после разработки
...
class FinishStep extends CWizardStep
{
	...
	function ShowStep()
	{
		...
		COption::SetOptionString("main", "wizard_solution", "personal", false, $siteID);
